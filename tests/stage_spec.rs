...
fn spec_bar070_上层槽恒可见() {
    // 槽序：[键行, AI, 配置, 文件树, 解析, 上层, 终端卡, 平移旧代, 平移新代, Demo]
    assert_eq!(
...
fn spec_bar083_z序_动者在上_五公民() {
    ...
    // 6. 不在栈者垫最底且次序确定（按 PANELS 声明序 Ai<Config<FileTree<Parser<Demo）
    assert_eq!(
