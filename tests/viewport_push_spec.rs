...
//! 与配置同约定（off ∈ [0,+w]，屏外右→靠泊），推移镜像配置。2026-09-26
//! 五公民：Demo 页入列（右缘家同约定，推移与解析/配置同支路）。
//!
//! 契约：
//! ①推移纯函数——五面板 off（已含缝采样/拖拽旁路）唯一决定基座页位移：
//!   AI 页落下 → 基座下移屏高；右缘家（配置/解析/Demo）进 → 基座左移
//!   屏宽；文件树镜像右移。面板全收（off 全在屏外位）= 基座回原位
//!   （零漂移）。
//! ②被压面板额外位移 = 其上各面板推移之和（交叉轴叠加 [配置,AI]：
//!   配置被 AI 压时随 AI 推移下移；顶面板永不受推）。
//! 变异抽检：①里把 push_y 写成 -（方向反）→ 题①红；解析家镜像符号反
//! （p_pt 写成 +）→ 题①解析臂红；Demo 臂符号反（p_demo 写成 +）→
//! 题①Demo 臂红；②漏加上方面板 → 题②红。
...
#[test]
fn spec_推移_ai落下基座下移() {
    // AI 屏外（off=-h）：基座原位、进度 0（其余四家全在屏外位）
    let (p, pmax) = viewport_push(-(H as i32), W as i32, -(W as i32), W as i32, W as i32, W, H);
    assert_eq!((p.dx, p.dy), (0.0, 0.0), "AI 屏外 = 基座不动");
    assert_eq!(pmax, 0.0);
    // AI 落一半：基座下移半屏
    let (p, pmax) = viewport_push(
        -(H as i32) / 2,
        W as i32,
        -(W as i32),
        W as i32,
    ...
    // AI 靠泊：基座整屏下沿
    let (p, _) = viewport_push(0, W as i32, -(W as i32), W as i32, W as i32, W, H);
...
fn spec_推移_水平双向镜像() {
    ...
    let (p, _) = viewport_push(-(H as i32), 0, -(W as i32), W as i32, W as i32, W, H);
    ...
    let (p, pmax) = viewport_push(-(H as i32), W as i32, -(W as i32), 0, W as i32, W, H);
    assert_eq!(p.dx, -(W as f32), "解析页靠泊 = 基座推出左缘");
    assert_eq!(pmax, 1.0);
    // Demo 页从右来（2026-09-26 五公民，右缘家同约定）：基座左移——
    // 变异：Demo 臂漏/符号反，本条红
    let (p, _) = viewport_push(
        -(H as i32),
        W as i32,
    ...
    assert_eq!(p.dx, -(W as f32) / 2.0, "Demo 页进一半 = 基座左移半屏");
    let (p, pmax) = viewport_push(-(H as i32), W as i32, -(W as i32), W as i32, 0, W, H);
    assert_eq!(p.dx, -(W as f32), "Demo 页靠泊 = 基座推出左缘");
    assert_eq!(pmax, 1.0);
    ...
    let (p, pmax) = viewport_push(-(H as i32), W as i32, 0, W as i32, W as i32, W, H);
...
fn spec_被压面板_只吃上方面板的推移() {
    let offs = (-(H as i32), W as i32, -(W as i32), W as i32, W as i32); // 全屏外
    ...
    // 同轴 [FileTree, Demo]（Demo 右缘家镜像）：Demo 进一半，文件树
    // （在下）被左推半屏——变异：Demo 臂漏/符号反，本条红
    let stack = [Panel::FileTree, Panel::Demo];
    let e = covered_extra(
        &stack,
        Panel::FileTree,
        offs.0,
    ...
    assert_eq!(e.dx, -(W as f32) / 2.0, "被 Demo 页压的文件树随其左移");
