...
//! 四修标定（细线 3px=细缘 W / R12=kfmv4 R:细线 4:1 比例 ×3 / 左线画
//! 框内缘不出框 / 线域 [20, 框宽−10−R]——线从弧尾 x0+R 起画，上限多
//! 收一个 R 线尾不越开口右缘）+ 五修标定（左粗 8px 实机观感；线色
//! 吃本卡 accent 渐变，涂装层钉）+ NA 随机线长机制（每次选中切换
...
fn spec_cursor_规格常量钉() {
    ...
    assert_eq!(
        EMPHASIS_W, 8,
        "左强调线宽 = 五修实机标定 8px（用户判 9px 粗 1px）"
    );
