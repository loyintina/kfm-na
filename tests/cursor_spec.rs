...
//! 2026-09-12 四修，宪法 §三/§四）
//!
//! 契约真相源：kfmv4 canvas-cursor 开口框形态（左粗/细线/圆角）+ NA
//! 四修标定（左粗 9px=装修框粗左缘 3×W / 细线 3px=细缘 W / R12=kfmv4
//! R:细线 4:1 比例 ×3 / 左线画框内缘不出框 / 线域 [20, 框宽−10−R]——
//! 线从弧尾 x0+R 起画，上限多收一个 R 线尾不越开口右缘）+ NA 随机
//! 线长机制（每次选中切换重掷，|顶−底| ≥ 1 格上下不对齐；窄框退化
//! 域内最大差）。
//!
...
fn spec_cursor_规格常量钉() {
    ...
    assert_eq!(EMPHASIS_W, 9, "左强调线宽 = 装修框粗左缘 3×W（四修标定）");
    assert_eq!(HAIR_W, 3, "发丝线宽 = 装修框细缘 W（四修标定）");
    assert_eq!(
        CORNER_R, 12,
        "圆角半径 = kfmv4 R:细线 4:1 比例 ×3（四修标定）"
    );
...
// ========== ②线域钉：顶/底线长各自 ∈ [min(20, w−10−R), w−10−R] ==========
...
fn spec_cursor_线域钉() {
    for (w, lo) in [
        ...
        (180, 20),
        (60, 20),
        (40, 18), // hi=18 塌到下限之下：lo 钳到 hi
        (30, 8),
        (24, 2),
    ] {
        let hi = w as i64 - LINE_MARGIN - CORNER_R;
    ...
    // 上限边界：线尾（弧尾 x0+R + 线长）距开口右缘恒 ≥10px（四修增补条款）
    for seed in 1..=100u64 {
        let mut rng = AccentRng::new(seed.wrapping_mul(0x77A1_5C3D));
        let g = roll(90, &mut rng);
        assert!(
            CORNER_R + g.top_w <= 90 - LINE_MARGIN && CORNER_R + g.bot_w <= 90 - LINE_MARGIN,
            "w=90 线尾越界（R+{} / R+{} 须 ≤ 80）",
            g.top_w,
            g.bot_w
        );
...
fn spec_cursor_上下不对齐钉() {
    ...
    // 窄域（w=60：域 [20,38] 幅 18）：skew = min(1格, 幅/2) = 9，
    // 构造保证恒可达（合法域恒非空）
    for seed in 1..=200u64 {
        let mut rng = AccentRng::new(seed.wrapping_mul(0x1B69_36B5));
        let g = roll(60, &mut rng);
        assert!(
            (g.top_w - g.bot_w).abs() >= 9,
            "w=60 半幅 skew=9 恒真（seed={seed} 得 |{}−{}|）",
            g.top_w,
            g.bot_w
        );
    }
...
fn spec_cursor_窄框退化钉() {
    for w in [0u32, 5, 10, 22] {
        let g = roll(w, &mut AccentRng::new(7));
        assert_eq!(
            g,
            OpenCursorGeom { top_w: 0, bot_w: 0 },
            "w={w} 框太窄必须双零（只画左线+圆角，不画发丝线；w−10−R≤0 即双零）"
        );
    }
    // w−10−R < 20 的边界带：下限塌到 hi，域幅 0 时两线相等是唯一解
    let g = roll(30, &mut AccentRng::new(7));
    assert_eq!(g.top_w, 8, "w=30 域塌成单点 [8,8]（hi=30−10−12）");
