...
fn spec_cfg标签栏_涂装钉() {
    ...
    // ⑤小圆角钉（2026-09-12 拍板：胶囊 36 → 小圆角偏方 12）：
    // 常量钉死 + 行为交叉证——顶缘近角（右 12px、下 2px 处）必须仍是
    // 环墨本体（红通道远高于底）；胶囊 36 会把这里切出矩形，只剩
    // c2 辉光薄墨（c2 红通道 = 0，blend 后红 ≈ 底 0x14）
    assert_eq!(
        kfm_na::termview::TAB_CURSOR_R,
        12,
        "光标框半径 = 12（kfmv4 R4:厚缘3 ≈1.33 比例换算）"
    );
