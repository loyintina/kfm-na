...
- **层级**：终端页恒在底（不收起）；其上两个面板公民——光球面板（AI 页，
  点球召唤，从顶落下）与配置页（左滑召唤，从右缘滑入；未来右滑家=文件树
  同规）。光球浮在两个面板之上（互斥召唤的开关不能断）。
...
- **抽屉对称手势**：滑向某面板的来向 = 先推它回家；该方向无面板才召唤
  对面家。v1：终端页/AI 页左滑 = 召唤配置页；配置页在顶时右滑 = 推回；
  配置页左滑 / AI 页右滑 = 留给右滑家（文件树），v1 空操作。
...
- **目标值语义在基础层**：面板在栈（顶或被覆盖）→ 靠泊位；不在栈 →
  ...
  目标值的一部分，插值仍在缝内。
- **实现锚点**（2026-09-10 落地）：状态核 `ai_presence.rs`（stack:
  Vec<Panel>，末位=顶）；手势识别纯函数 `decide_swipe`（90px 阈值 +
