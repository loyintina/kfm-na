...
- **焦点二态**：终端 / 输入栏（点输入栏聚焦，Esc 或点终端区失焦）——组件三落。
  栏聚焦时按键全归栏，**方向十字例外直通终端**（BAR-111：栏内 v1 无方向
  语义，吞了 = 死键——键盘在场时快捷键行方向键点按/长按连发全灭的真凶；
  分流纯函数 `ime_queue::split_bar_focus`）。
...
- **层级**：终端页恒在底（不收起）；其上四个面板公民（2026-09-12
  四公民落地·三缘语义）——光球面板（AI 页，点球召唤，从顶落下，
  **顶缘 = 全局 AI**）、文件树页（右滑召唤，从左缘滑入；占位壳先行，
  **左缘 = 路由**：切换中央对象）、解析页（左滑召唤，从右缘滑入；
  青蓝占位壳先行，**右缘 = 解析器家族**：解析中央对象；池卡设置也
  算解析家族但从右缘齿轮进——见下条）与配置页（设置页，独立第四页，
  见下条）。光球浮在四个面板之上（互斥召唤的开关不能断）。
- **设置页独立（2026-09-12 用户拍板，同日第三拍定稿四页模型）**：
  设置页（配置面板）不占任何滑槽——**齿轮按钮（`ui/gear.rs`，终端页
  右上角）是唯一召唤口，右滑（含跟手拖拽）是唯一关闭口**。点钮只召
  不收（顶=Config 时点钮 = 幂等空操作，无 toggle）。左滑槽让位给解析
  页。按钮画进终卡槽，任何面板靠泊时随整层隐没（slot_visibility 白拿
  可见性），天然满足「文件树/解析页在顶时按钮不出现」。按钮在顶时仍
  只认光球。（同日时间线：早批两拍冻结左右滑槽 → 午批解冻回三公民
  全活 → 午后拍板四页模型定稿。）
- **文件树页（左缘 = 路由）**：右滑召唤槽——裸终端/AI 页右滑 = 召唤
  文件树。机制纯手势驱动（`force_role_for_spec` 旁路已退役删除）。
  浏览器未来是中央对象，入口在路由侧（左缘家族）。
...
- **叠加态坍缩**（用户原话的机制化）：被覆盖 = 「覆盖×收起」叠加态——
  ...
  丢弃无感）——例（四公民全活时的走查）：
  AI 页上右滑出文件树→点球（AI 盖文件树）→左滑出解析页
  （文件树被挤出）→关解析页（AI 露出）→点球收 AI 页→**露出终端**。
- **视口平移（2026-09-12 用户拍板，`ui/viewport_push.rs`）**：
  ...
  **Q 弹形变同日二审取消**（实拍不合预期——squash 弹簧退役，
  ...
  语义（不再投入的既有档位）。
- **抽屉对称手势**（2026-09-12 四公民·三缘语义定稿：任意栈态每个滑向
  有唯一归宿，不留语义洞）：滑向某面板的来向 = 先推它回家；本家已在顶
  = 空操作（一滑一义）；该方向无面板可推才召唤该方向的家。设置页不占
  召唤滑槽（齿轮唯一召唤口），但占右滑关闭（与解析页同一家规）。
  | 滑向 | 顶=文件树 | 顶=配置（设置页） | 顶=解析页 | 顶=AI 或终端 |
  |---|---|---|---|---|
  | 左滑 | 推回文件树（左缘） | 空操作 | 空操作（本家在顶） | 召唤解析页（右缘家） |
  | 右滑 | 空操作（本家在顶） | 推回配置页（右缘，唯一关闭口） | 推回解析页（右缘） | 召唤文件树（左缘家） |
- **跟手拖拽**（interactive gesture-driven transition，2026-09-11 用户
  ...
  横向三家全覆盖（2026-09-12 四公民：左缘家=文件树，右缘家=解析页——
  SummonConfig 变体删除（设置页永不走手势召唤），原位 SummonParser，
  偏移/甩速/裁决符号语义与旧配置系完全一致）；AI 页竖向落下不走跟手。
...
- **实现锚点**（2026-09-10 落地，09-11 三公民泛化，09-12 四公民）：状态核
  ...
  **BAR-084 配套红线：z 序活性读数只许进 z 序，target/presence 走
  `panel_target_and_draw` 单源（target 只问栈、draw 才看活性）**；
  机器轨 = stats 字段 `panel_top`/`panel_cov`（none/ai/config/filetree/
  parser）+ `ai_epoch`/`cfg_epoch`/`ft_epoch`/`pt_epoch` + na-shot 倒帧
  装帧含四面板（gate dump_now 与前台 paint_under 同 z 序）。
