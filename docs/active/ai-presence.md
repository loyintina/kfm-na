...
### 五B、面板栈模型（2026-09-10 用户拍板，D12；配置页壳先行）

- **层级**：终端页恒在底（不收起）；其上三个面板公民（2026-09-11
  三公民落地）——光球面板（AI 页，点球召唤，从顶落下）、配置页
  （**2026-09-12 起：设置钮召唤**，从右缘滑入——见下条）与文件树页
  （右滑召唤，从左缘滑入；占位壳先行，内容后填）。光球浮在三个
  面板之上（互斥召唤的开关不能断）。
- **配置页入口改设置钮（2026-09-12 用户拍板）**：终端页右上角齿轮
  钮（`ui/gear.rs`）是配置页**唯一入口**——点钮从右侧滑出全屏配置页，
  右滑（含跟手拖拽）推回收起。按钮画进终卡槽，任何面板靠泊时随整层
  隐没（slot_visibility 白拿可见性），天然满足「文件树/浏览器在顶时
  按钮不出现」。原「左滑召唤配置页」槽**冻结**留给浏览器卡
  （SPKE-web 解冻时启用）：裸终端左滑现为空操作。按钮在顶时仍只认
  光球——与「配置页在顶不能召文件树/浏览器、只能召光球」的拍板一致。
- **文件树页冻结（2026-09-12 用户拍板，同日二拍）**：文件树公民保留
  在栈模型里（`summon_panel(FileTree)` 编程入场、被盖/挤出/推回语义
  全部留档），**手势召唤槽关闭**——裸终端/AI 页右滑现为**空操作**。
  冻结期手势面只剩：点光球召/收 AI 页、设置钮召配置、配置/文件树顶
  的右滑/左滑推回。机制钉（跟手映射/松手裁决/镜像）改走
  `force_role_for_spec` 旁路注入保活，解冻时撤注入回纯手势驱动。
- **每面板三态**：`收起` / `在顶` / `被覆盖`。不变量：至多一个在顶；
  栈至多两格（一个顶 + 一个被覆盖）。
- **叠加态坍缩**（用户原话的机制化）：被覆盖 = 「覆盖×收起」叠加态——
  ①遮盖者退场时坍缩为「被覆盖」：**随视口推移滑回**（2026-09-12 视口
  平移拍板后改写——旧规「直接露出不播动画」作废，见下条）；②再次被
  召唤时坍缩为「收起」：重播入场动画盖到顶；③第三面板
  入场栈溢出时，栈底被覆盖者坍缩为「收起」：**静默挤出**（本来不可见，
  丢弃无感）——例（三公民全活时的走查；2026-09-12 起左右滑召唤槽均
  冻结，文件树/浏览器经 summon_panel 编程入场时同一套栈语义不变）：
  AI 页上右滑出文件树→点球（AI 盖文件树）→左滑出配置页
  （文件树被挤出）→关配置页（AI 露出）→点球收 AI 页→**露出终端**。
- **视口平移 + Q 弹形变（2026-09-12 用户拍板，`ui/viewport_push.rs`）**：
  面板交互从「覆盖」改「视口平移」——原内容平移走、新页面移过来。
  推移是纯合成期派生：三面板 off（缝采样/拖拽旁路后的渲染值）唯一
  决定各层 placement，**状态核目标值体系零改动**。基座页（终端卡槽 +
  网格实例 + 键行槽）随面板进度平移；被压面板吃上方各家推移之和
  （交叉轴 [配置, AI]：配置随 AI 下移）；跟手期底页随动零新机制
  （拖拽旁路改写的就是 off）。Q 弹 = 欠阻尼弹簧积分器驱动压缩量
  （scale = 1 - s，压满 6%、斜坡前 22% 进度）：被挤走的页先轻微形变
  再出场；收尾部目标骤降 → 惯性过零 → scale 短暂 >1「墩一下」。
  输入栏与光球是全局件，永不平移。softbuffer 兜底路径保留旧覆盖
  语义（不再投入的既有档位）。
- **抽屉对称手势**（2026-09-11 三公民定稿：任意栈态每个滑向有唯一
  归宿，不留语义洞——用户拍板「占位页也先做出来，手势才能唯一不冲突」）：
  滑向某面板的来向 = 先推它回家；本家已在顶 = 空操作（一滑一义）；
  该方向无面板可推才召唤该方向的家。
  | 滑向 | 顶=文件树 | 顶=配置 | 顶=AI 或终端 |
  |---|---|---|---|
  | 左滑 | 推回文件树（左缘） | 空操作 | 空操作（2026-09-12 冻结：配置召唤归设置钮，此槽留给浏览器卡） |
  | 右滑 | 空操作 | 推回配置页（右缘） | 空操作（2026-09-12 二拍冻结：文件树召唤槽关闭） |
- **跟手拖拽**（interactive gesture-driven transition，2026-09-11 用户
  拍板手势升级，`panel_drag.rs` 状态机）：横向位移过 24px+1.8 方向锁即
  接管手势（先于 decide_swipe 的 90px 抬手判定——抬手识别降级为「无
  move 事件的 tap 位移」边例兜底）；拖拽期面板偏移绑手指不绑时间（渲染
  旁路缝采样直读 `current_offset`，手指停画面停零插值滞后），**位移
  ×DRAG_GAIN=2 增益**（同日二审拍板：手指不从屏边缘起手，手指 10px
  面板 20px，半程手势走完全程）；松手按
  「甩速 0.8px/ms（100ms 窗）优先、进度 50% 次之」裁决完成/取消——
  反悔回拉快甩 = 强制取消，面板回去不出场；收尾用缝 replay 踢
  （BAR-079 原语）从当前跟手偏移重定基续播，无缝接管。锁定瞬召唤拖拽
  立即入栈（渲染靠栈存在性；新鲜召唤不 bump 入场代，无 replay 竞态）。
  横向两家全覆盖（2026-09-11 三公民：文件树家镜像，裁决/甩速符号
  按家翻转）；AI 页竖向落下不走跟手。
  **同日二审配套：面板淡入淡出全取消（恒 1.0）+ 定时动画提速
  250/180ms。**
- **目标值语义在基础层**：面板在栈（顶或被覆盖）→ 靠泊位；不在栈 →
  屏外位。动画只许在缝内插值（ui-base §三铁律不变）。**BAR-079 修订：
  目标值 =（位置, 入场代）**——同一栈态有两种历史（新鲜召唤/覆盖再
  召唤），纯位置函数算不出「该动」；状态核对被覆盖再召唤/被挤出 bump
  入场代，壳层见代变踢缝重定基到屏外位 → 重播入场/静默瞬移。代只是
  目标值的一部分，插值仍在缝内。
- **实现锚点**（2026-09-10 落地，09-11 三公民泛化）：状态核 `ai_presence.rs`
  （stack: Vec<Panel>，末位=顶）；手势识别纯函数 `decide_swipe`（90px 阈值 +
  1.8 方向锁，抬手判定——Started 分流优先级：球 > 选择菜单 > 输入栏
  > 面板页 > 键行 > 终端区）；配置页渲染 `paint_cfg_page_chrome`
  （青底 0x0A1A20 + 青系边框环，与 AI 紫底 0x140A24 机器判卷可区分）、
  文件树页渲染 `paint_ft_page_chrome`（墨绿底 0x000A1A0F + 绿系边框环，
  三公民底色机器可区分）；GLES 第四/五槽 ChromeSlot::Config/FileTree，
  z 序单源 `stage::panel_z_order`（**BAR-083 修订：动者在上**——
  旧规逐帧跟 snap.top，撤 AI 瞬 AI 出栈、不透明配置页当场压顶，AI
  退出动画在它背后播完 = 瞬消；新规：动画/拖拽中的面板压顶，多动者
  按栈序，双静止跟栈顶露出零动画承载不变）；**BAR-084 配套红线：
  z 序活性读数只许进 z 序，target/presence 走 `panel_target_and_draw`
  单源（target 只问栈、draw 才看活性）**——活性泄漏进 target = 退场
  面板回粘靠泊位（栈空页满两张皮）；机器轨 =
  stats 字段 `panel_top`/`panel_cov`（none/ai/config/filetree）+
  `ai_epoch`/`cfg_epoch`/`ft_epoch` + na-shot 倒帧
  装帧含三面板（gate dump_now 与前台 paint_under 同 z 序——首日实机
...
- **期 0（2026-08-30 用户拍板重排 D10 + 纠偏 D11：先接通真 AI，浮层/手靠后；
  ...
  **③样式修订（2026-08-31 下午，用户实拍判卷驱动）**：复刻 kfmv4
  参考图（实拍取色）——文本区浮在带内（带高 132→220，上下留白 32、
  左右离屏边 60），紫→青渐变描边 3px + 暗蓝底；发送钮独立圆角方块
  （140 宽、紫→青对角渐变 + 白 ▶ 三角图元），占位文案「输入消息…」。
  ...
  **④先行（2026-09-04，AI 页交互地基四件套，用户实看拍板）**：
  ①**不穿透**——面板靠泊时触摸路由提前分流（android_app Started
  臂）：面板上只有输入栏是活区，其余手势归对话页；终端手势全家
  （含快捷键行幽灵热区）让路。②**点非输入区收键盘**——AI 页点按 =
  输入栏失焦 + insets.force_hide（ImeInsets trait 新方法，JNI
  hideSoftInputFromWindow），绝不穿透召唤终端输入法。③**视口滚动**
  ——ui/ai_page.rs AiPageScroll 状态机（追底/上滑取消/滑回恢复/
  钳制，A 档 9 题），渲染 render_ai_page 带 scroll_rows 返回布局
  写回（眼手同尺）。④**思考块**——消息模型加 thinking 第三字段
  （BAR-059 分账从「收流即弃」改「随消息存档」），渲染 ≤3 行暗色
  尾随窗（thinking_window 纯函数 + AI_THINK_FG），流式增长窗口
  跟尾 = 用户要的「两到三行里自己滚动」；kfmv4 式折叠块退役。
  **④判卷修订（2026-09-04 同日复判，四条）**：①**发送即跳页**——
  输入栏发送闭包加 tap_overlay：终端页发完直接转进 AI 面板看生成，
  不许停终端页等手动点球。②**思考收流折叠**——live_tail 语义：
  流式中的末条保持 ≤3 行活窗；收流后思考折叠成一行暗色
  「· 已思考 ·」（AI_THINK_COLLAPSED），全文随消息存档不丢——
  「思考往往不重要但必须存在」。**折叠时机再判（同日二判）**：
  思考一结束立即折，不等整轮收流——live_tail 数据源从
  is_streaming（整轮）改 thinking_live（思考相位：流式开着且
  正文零字节；首个 TextDelta 落地即翻 false，kfmv4 首块正文
  折叠同判据），A 档翻转钉 + 变异咬。③**键盘让位追底**——render_ai_page
  加 bottom_inset（键盘高 + 输入栏当前带高），视口下沿抬到栏带
  上沿，追底不许越过栏带往下画。④**开合自追**——键盘弹/收只改
  fit，既有 sync_layout 追底态自动贴新底（零新代码，状态机语义
  天然覆盖）。A 档两新钉（折叠一行/inset 收紧）+ 变异双咬。
...
⑤ 「· 已思考 ·」点击展开（契约已备：折叠尊重用户——手动展开过的
   块自动折叠不得覆盖，kfmv4 _foldState 同规；参考 工具卡.md §七
   登记的聊天面板解剖 SEC 04/05）
...
【D12】配置页壳与面板栈模型（2026-09-10 用户拍板）
决定：①终端页全局输入栏**保留**（翻案撤回当日早些时候的「输入栏迁入 AI 页」
草案——终端直接问 AI、未来文件读写都要全局入口）；②新增配置页（API 接入
管理，v1=全屏框+空白背景骨架，内容后填），左滑召唤/右滑推回（nz 同构）；
③面板栈模型落定（§五B：三态+两格栈+叠加态坍缩+抽屉对称手势），光球面板
与配置页互斥，未来文件树（右滑家）同规入栈；④实现序列：骨架页+手势+栈
逻辑先行（自验=闸门注入驱动+stats 字段族+GPU 回读实拍双轨），provider/
会话管理内容后续填
理由：本地 AI「能跑→能用」的完成态需要配置 UI 化；互斥栈用「叠加态坍缩」
一词收编了所有边界案（再召唤=重入场/遮盖撤=露出/溢出=静默挤出），不留
特例；手势抽屉对称保证一滑一义
否决案：输入栏迁入 AI 页（杀全局入口）；纯栈模型不挤出（三面板时露栈底）
标签：方向确立

【D13】入场代：坍缩②的物质基础（2026-09-10 实机帧级仪器实踩后修复，
BAR-079）
决定：①§五B 目标值语义从「位置」升级为「（位置, 入场代）」——状态核
`ai_presence.rs` 每面板一枚 epoch，只在被覆盖者离被覆盖位时 bump
（再召唤=②重播/被挤出=③瞬移）；新鲜召唤/退场/露出都不 bump（目标
翻转自足/打断不跳变/零动画露出）。②缝占槽件加 `replay` 重播踢原语
（seam.rs Occupier 第三件；fx_ease 实现，fx_spring 键盘 inset 无入场
概念 = None）——踢 = 采样器重定基 from=屏外位，目标不动，插值仍在
缝内（铁律不破）。③壳层 `poll_ai_presence` 机械中继（代变 → 踢），
契约判定零入壳。④stats 加 `ai_epoch`/`cfg_epoch`——「重播入场发生
了」从此是机器轨直读字段，不再靠帧差分间接推断
理由：实机动画监控首日实战即破案——覆盖再召唤时帧差分 +1、anim-cap
触发未消费，而状态级判卷（栈字段+截图）前一天全绿：同一栈态两种历史
是位置纯函数的数学盲区，补丁（壳层特判踢）会把契约知识漏进胶水，
文件树面板进来还得再糊一次；入场代把这一比特历史显式编码回状态核，
纯函数重新成立，A 档考题+变异四咬钉死
否决案：壳层 kick 补丁（契约漏进胶水，下一块面板再糊一次）；被覆盖=
屏外位（坍缩①的零动画露出失去物质基础：遮盖退场的中途帧会露出终端
而不是被覆盖者）
标签：打脸结晶（仪器判据首战功：状态级判卷全绿 ≠ 动画播了）

——kfm-na(Kimi Code) · 2026-08-30 立 · v3 两布尔模型+雾球入档 · 实现以期为单位，每期过 chain+regress+实拍

【D12】AI 页装修：边框 + 进出场定时缓动（2026-09-04 用户拍板）
决定：①AI 对话页加边框——仿 kfmv4 对话面板（orb.ts createPanel 的 CSS
配方直译）：135° 渐变描边（青 .8 → 紫 .7）+ 左缘 3 倍粗 + 圆角 12 CSS
px + 紫外发光，外缘距屏边 16px、下缘停在输入栏带上沿之上（对话框坐在
输入栏上的游戏对话框语言）；空态也画（框是装修不是内容）。②面板进出场
动画从欠阻尼弹簧改定时缓动：落下 ease-out、收起 ease-in
（CSS transition 语言）——新件 src/ui/fx_ease.rs 占「AI 面板 Y 偏移」缝；
弹簧（fx_spring.rs）退役到「键盘 inset」缝独占。时长初定 500/400，
同日实测定档 350ms/250ms（用户复测「改成下落350ms，收起250ms」）
理由：用户实拍拍板「仿照 kfmv4 的对话面板给 ai 对话页面做装修」「下落用
500ms 的 ease-out，收起用 400ms 的 ease-in」
否决案：弹簧墩感（当日上午方案，实拍后用户改判）；边框带墨接入空态零墨
契约（空态零墨的原意是撤占位提示文字，框是页面装修——ai_presence_spec
两道冒烟钉已按新契约改写：框内内容区零墨照旧判、边框带必须有墨）
...
考题增至九题。
2026-09-06 定稿：emphasized 仍被判「不符合直觉」（用户实看）——换
**物理重力**：落下 = 自由落体 t²（起步静止、线性加速、砸到底，匀加速
精确解非近似），收起 = 镜像 1-(1-t)²（减速上升），时长 350/250 照旧。
同批 ras 带切优化（环/punch 整包围盒 SDF 260 万像素 → 九宫格带切，
chrome 40.7→7.1ms；输入栏 glow 同款切带）——动画掉帧与曲线手感
两个病灶同批落地。考题：重力物理签名（10% 时间 1% 路程）+ 半程方向
锁定 + 存量形状题按新契约改写。
2026-09-11 定稿：重力 t² 被用户录屏逐帧实锤判「冻结→跳变」——
实测帧泵 ~100-120Hz 没偷懒，病灶在曲线本身：t² 起步亚像素蠕动 +
alpha 从落程推导跟着 t² 走，前 100ms 面板近乎透明，显影时已在加速段。
换 **power2.out**（1-(1-t)²，nz/kfmv4 的 GSAP 同款减速）：起步即快
（10% 时间走 19% 路程，第一帧就有可见位移）、缓停到位。alpha 机制
一行不动（仍从落程推导），落程快涨带动 alpha 前 ~19% 时长即全实
（1-√0.65≈0.194），连带治好显影延迟。AI 面板落下与配置页滑入同
吃此曲线（fx_ease::power2_out / panel_ease_pos 进场臂）。考题换装：
签名题改 10%→19%、90%→99% + 半程判定翻转 + fade 咬合题语义翻转
（起步即显影 a>0.5、20% 时长近乎全实）；变异双咬（改回 t² 咬 4 题、
改 1-t² 咬 5 题含端点）。

【D14】三公民面板栈落地：文件树占位页先行（2026-09-11 用户拍板）
决定：①不留手势语义洞——文件树面板占位壳（墨绿底 0x000A1A0F+绿系
边框环，内容后填）与手势契约同批落地，三公民（AI/配置/文件树）任意
栈态每个滑向有唯一归宿（滑向契约表见 §五B）；②z 序单源泛化
`stage::panel_z_order`（不动者按栈序、动者压顶、多动者按栈序，
不在栈者按 PANELS 声明序垫底），panel_z_cfg_on_top 退役；③配套根治
BAR-084——target/draw 单源 `panel_target_and_draw`（target 只问栈、
draw 才看活性），z 序活性读数只许进 z 序；④拖拽偏移语义统一为「距
靠泊距离」∈[0,w]，壳层按家折符号（配置 +off / 文件树 -off）；
⑤GLES 第五槽 ChromeSlot::FileTree，softbuffer 兜底保持硬切
理由：用户原话「干脆把文件树的面板占位也先做出来，三张页面一起做，
这样测试就是完整的，行为理顺不会冲突——它是真实存在而不是空位置，
手势就能唯一不会冲突」；redroid 实测 BAR-084 实锤「活性进 target」
回粘回路，同批根治不打补丁
否决案：右滑留空操作洞等文件树内容就位再补（两公民期「AI 页右滑零
响应」已被用户场景走查证伪——手势语义洞会让栈状态机不可测）
标签：方向确立+打脸结晶（BAR-084：修 BAR-083 引入的活性读数用法本身
构成新回路——单源边界划错一处，修复变病灶）
