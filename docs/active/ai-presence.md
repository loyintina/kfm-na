...
- **期 0 骨架+假 AI**：echo-brain（本地回显零网络，BrainEndpoint 首个实例）。
  四组件分期，每组件独立考题+热更+实拍：
  ① 状态核+光球（切换钮/指示灯/拖动/按压/AI 遥控/探针观测注入；AI 页=占位空壳）
  ② 浮层绘制+echo 桩（自动现/驻留/自动隐/甩掉）
  ③ 全局输入栏+发送按钮（真实触发源）
  ④ AI 页合成网格（markdown-lite 经 `dyn TermEmuFactory`）
  验收 = 实拍：点球切页 + 拖动换位 + 假运行灯亮灯灭 + 浮层尾随 + 发送回显。
...
标签：外部判据（C 档真机实拍可调）
状态：档位部分随 D7 废止；位置部分保留为默认出生位（D9 起可拖走）
...
【D8】光球视觉 = 雾状光球 sprite（kfmv4 血统）
决定：抄 kfmv4 base.scss:23 原配方——radial-gradient(circle at 30% 30%,
#7C3AED .9→.4→transparent 70%) + 双层光晕，程序化预渲染 sprite（运行时纯贴图）；
整体 alpha 压缩（闲 ~25% / 运行 ~50%+晕略扩）；pressed（按下/长按/拖动中）= 光晕
加大亮（kfmv4 :active 同款硬切）；人在 AI 页 = 核高光略增强；透明度总系数常量可调
理由：用户首要诉求 = 几乎透明不挡后面内容、但确实可见有球；原版的球感全靠径向亮度
落差 + 光晕对比（无轮廓线），压 alpha 既保血统又保底下可读；零动画帧合 ui-base 纪律
否决案：实心渐变盘（覆盖率 100% 挡字）；空心细环（非原版血统，讨论中提出后被否）
标签：外部判据（C 档真机实拍可调）
