...
> 状态：设计进行中（2026-08-30 立）。当前期：期 0 未动工。
...
- **常驻 chrome 二 · 光球 = AI 存在感三档旋钮**：隐身（纯终端）/ 伴随（浮层）/
  主场（AI 页），点按循环；AI 忙/闲/未读 → 静态换图报信，零常动帧。
- **伴随浮层**：顶部 1/4，75-85% 不透明暗底，消息尾随上滚 = **同一聊天网格的
  第二视口**（尾随锁定，零状态复制）；v1 纯显示、触摸穿透。
...
- 待定③：发送后的档位行为——建议：隐身 → 升到伴随，不强制主场（短回复浮层读
  完，长回复点光球）。替代案：纯文字回复自动进主场。待拍板。

## 六、分期（每期独立可验收，慢慢来）

- **期 0 骨架+假 AI**：echo 后端（本地回显零网络）。三档旋钮/浮层视口/全局输入栏/
  AI 页全部就位。验收 = 实拍：三档切换 + 浮层尾随 + 发送回显。
- **期 1 真协议**：读 kfmv4 源码 → 契约回填 → 解析器考题 → 对接。验收 = 真机问答一轮。
- **期 2 眼睛**：视口快照附上下文。验收 = 问「屏幕上有什么」答得对。
- **期 3 手**：工具执行注入来源会话。验收 = 让 AI 跑一条命令，终端可见。
- **期 4 打磨**：工具调用独立悬浮框 / 展开动画（ui-fx 第二插件，缝的首批属性落位）/
  浮层可触摸滚动。
...
③ 发送后档位行为（建议：隐身→伴随，不强制主场）
④ 浮层几何参数（高度比例/透明度/圆角）——C 档真机调
⑤ 光球默认档位/默认位置；AI 会话在 Ctrl-] 切换环的位置
⑥ 消息本地持久化格式（飞行记录仪风格）
...
——kfm-na(Kimi Code) · 2026-08-30 立 · 实现以期为单位，每期过 chain+regress+实拍
