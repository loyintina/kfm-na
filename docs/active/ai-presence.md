...
- **层级**：终端页恒在底（不收起）；其上三个面板公民（2026-09-11
  ...
  （左滑召唤，从右缘滑入；另有设置钮入口——见下条）与文件树页
  （右滑召唤，从左缘滑入；占位壳先行，内容后填）。光球浮在三个
  面板之上（互斥召唤的开关不能断）。
- **配置页双入口（2026-09-12 用户拍板，同日三拍解冻）**：设置钮
  （`ui/gear.rs`，终端页右上角齿轮）与左滑手势**并存**——点钮/左滑
  都从右缘滑出全屏配置页，右滑（含跟手拖拽）推回收起。按钮画进终
  卡槽，任何面板靠泊时随整层隐没（slot_visibility 白拿可见性），
  天然满足「文件树/浏览器在顶时按钮不出现」。按钮在顶时仍只认
  光球——与「配置页在顶不能召文件树/浏览器、只能召光球」的拍板
  一致。（同日曾两拍冻结：左滑槽留浏览器卡、右滑槽关文件树；
  当日晚用户要实测三公民叠加交互，双槽一并解冻回 f5cad18 原版
  语义——浏览器卡入口重新悬空，待 SPKE-web 立项再定槽。）
- **文件树页解冻（同日三拍）**：右滑召唤槽恢复——裸终端/AI 页右滑
  = 召唤文件树。机制钉（跟手映射/松手裁决/镜像）撤回
  `force_role_for_spec` 旁路注入，回纯手势驱动（该方法已退役删除）。
...
- **叠加态坍缩**（用户原话的机制化）：被覆盖 = 「覆盖×收起」叠加态——
  ...
  丢弃无感）——例（三公民全活时的走查；2026-09-12 同日三拍解冻
  后手势面恢复全活，此走查链全程手势可达）：
...
- **抽屉对称手势**（2026-09-11 三公民定稿：任意栈态每个滑向有唯一
  ...
  |---|---|---|---|
  | 左滑 | 推回文件树（左缘） | 空操作 | 召唤配置页（2026-09-12 同日三拍解冻，与设置钮双入口并存） |
  | 右滑 | 空操作 | 推回配置页（右缘） | 召唤文件树（同日解冻恢复） |
