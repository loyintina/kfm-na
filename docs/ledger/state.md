...
> **结案批（09-18 用户肉眼终验通过「可以结案了」）**：十五修（动效预览
> 10 件）/ BAR-104（贴死交接闪变）/ BAR-105+106（起步左闪+贴死左冲）/
> BAR-107（框动行不动+半透明芯）/ BAR-097（池高拆层+节流）全部结案。
> 下一主线候选：nz C 线审计信回信（欠 5 天）；设置页服务器配置池
> 下拉选中态尾巴。
...
- **解析页 tmux 插件 v1（2026-09-19 用户立项「右侧 tmux 插件加窗口
  ...
  kfm-na -x <na格宽> -y <na格高>`（manual 下立即生效，窗口重排到
  手机网格）。插件形态：解析页一张二级卡「tmux · <会话>」——窗口行
  列表（点行 = select-window，附着终端自动跟随；行尾 × = kill-window）
  + 底部按钮 [重排] [+新窗] [↻刷新]；本地会话活跃 = 占位「本地会话」。
  控制通道 = 短命 ws 会话跑 `tmux …; exit` 收输出（服务端零改动，
  复用 conn.rs Session 状态机）。热更 .so 即插拔。
