...
> **服务卡 v1 落地（2026-09-20，待提交+redroid 判卷+真机终验）**：
> 解析页第三张二级卡（na-server.md §四）——卡头「服务 · 状态词」+
> 四字段行（后端/在线/会话/错误）+ 分隔线 + 每会话一行，v1 纯展示。
> 新册两本：`src/ui/svc_card.rs`（三源合成文案 + 动态高几何，A 档
> 13 考题，变异三枚全咬：inset 漏间距/卡高漏行距/Error 清旧数据）+
> `src/svc_health.rs`（health 轮询器：解析页可见 2s 一拍不可见不轮，
> http1 手写客户端打隧道本地口零新依赖；Error 相保留旧数据闪断不清
> 卡面）。壳接线：五处 inset 扩席同源（inset_extra_live 与涂装同
> 快照）、卡区手势仲裁吞服务卡（纯展示也不许漏成滑页起手）、
> set_visible/take_dirty 挂主循环。组件池已登记。**判卷标准**：
> redroid 截屏三卡纵排 + 会话行与 `curl health` 对表 + 后端切
> kfmv4 显托管态。
