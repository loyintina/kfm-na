...
## 当前位置（2026-09-25 午，tmux 外置视口像素滚动落地，待用户终验）

> **tmux 像素级滚动（外置视口）已实现待判**（设计契约
> docs/active/设置页.md §3.5）：「一次滑闪 5 行」真凶 = tmux 默认
> 绑定 -N 5（服务器 list-keys 实证），滚轮 tick 路救不了对端 5 倍率。
> 根治 = tmux 附体拖动起手抓 capture-pane 全史快照喂独立 browse
> Term，渲染/滚动/命中切源直滚快照（与终端同款 scroll_px 机制，
> 1:1 像素跟手全史可滚）；抓取往返期位移挂账切入补滚；live 照常
> 喂，击键/切会话/attach/resize/鼠标位消失即退回最新。判卷点：
> 拖动起手 ~300ms 切入浏览，之后像素跟手；击键即回 live。钉
> termview_spec 浏览态三钉 + tmux_ctl_spec cmd_capture（变异三咬
> 全中）。BAR-151③ 灵敏度挂账随之根治（不再走滚轮 tick）。
>
