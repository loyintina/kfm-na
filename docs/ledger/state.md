...
- **BAR-108：挂起态触摸几何盲区（2026-09-17，已修待 vc454 判卷）**：
  ...
  连带同病臂：齿轮/键栏/输入栏命中 + 面板拖拽宽。判卷口径：vc454
  热更后挂起态注入 tap 关跳框，须见「跳框收起」留痕+rgb 截屏无框。
- **十五修：动效预览乒乓化+语义化（2026-09-17，用户拍板「一起做吧」，
  已实现待判卷）**：组件池「动效引擎」10 件跳框预览重构。病史 =
  ...
  **下一步：vc453 真机热更 + na-rec 实录判卷，请用户肉眼终验**
