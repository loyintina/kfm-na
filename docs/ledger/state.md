...
> （BAR-114 升代码守卫 PARADIGM v2、BAR-116 挂账理由）。**新方向
> （用户问「9022 能不能推？我们自己的推送路径？」）**：9022 设计
> 上就是「反连直达 NA sshd」（与 8024 同 sshd 双入口），但 v1 把
> -R 划给 Termux 维护 → Termux 冻结它就不存在。正解 = -R 并进 na
> 自持隧道（tunnel.rs forward_args 现在只发 -L；settings 的
> remote_port=9022 字段留位没人用；手机本地 sshd 实锤听
> 127.0.0.1:8024/kfm-sshd.conf）——na 看门狗双腿（BAR-117）维护，
> na 活着反连就在，推送不再依赖 Termux。实现推进中。
>
