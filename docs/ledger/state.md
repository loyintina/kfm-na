...
- **BAR-074 幂等闸前置（同日，PIN-boot 挂卷族真凶）**：壳每启裸读 32MB
  bootstrap 资产后才问 ensure_prefix（它第一行就跳过）——启动关键路径
  白读 32MB，IO 一挤 boot 段 3s+。field-reports 回查：慢启动 08-21 起
  震荡至今（L3 落地次日），与 BAR-072 无关。修：prefix_ready 谓词
  单源核心层，壳读资产前先问。判卷=热更后 PIN-boot 回绿。
