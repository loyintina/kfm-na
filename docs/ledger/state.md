...
- **立案：键盘弹起改视口平移（2026-09-18 晚，用户拍板「就这样做吧」）**：
  ...
  看历史（display_offset>0）时 shift=0 不打扰阅读。判卷仪器预案：
  tmux 侧 `#{window_width}x#{window_height}` 键盘弹收全程不变（铁证）
  + na-rec 帧账差分应为纯平移无重排。真实窗口尺寸变化（横竖屏/
  分屏/捏合）照旧重排，不受影响。
