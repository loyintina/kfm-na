...
## 当前位置（2026-09-25 午②，外置视口 v2 后台模型重写落地，待编核热更+用户终验）

> **v2 后台模型**（设计契约 docs/active/设置页.md §3.5，用户否决
> v1 三点后拍板）：①零感知切入——拖动起手用温热快照即进浏览态
> （闲时/抬手/退场后台抓好的），抓取只作后台刷新，无 0.3s 等待；
> ②浏览期动态播放——浏览态非静态：1.5s 节流 + feed_seq 代际比对
> 有新活动才重抓，swap_browse 原地刷新，视口锚从底部量守恒
> （阅读位不跳）；③触底自动回 live——display_offset=0 且零头清零
