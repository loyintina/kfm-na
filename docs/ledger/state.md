...
- **视口平移 + Q 弹形变（2026-09-12 上午，用户拍板）**：面板交互从
  「覆盖」改「视口平移」——原内容平移走、新页面移过来；被挤走的页
  先轻微整体形变（压满 6%，前 22% 进度斜坡）再出场，收尾部欠阻尼
  弹簧惯性过零「墩一下」。新模块 `src/ui/viewport_push.rs`（纯函数：
  三面板 off → 基座推移 + 被压面板额外位移 + 形变目标 + Q 弹积分器；
  半隐式欧拉，ω=22 ζ=0.55），应用点在 draw_frame_gles：基座实例过
  ...
  动画露出」变「随推移滑回」。帧时钟第四路活性源 squash_active
  （fx_frame_due）。跟手期底页随动零新机制（拖拽旁路改写的就是 off）。
  考题：viewport_push_spec 5 钉 + glyph_atlas_spec 推移钉，变异四咬
  （方向反/漏累加/删 clamp/速度丢 dt 全抓）。**欠账：softbuffer 兜底
  路径保留旧覆盖语义**（不再投入的既有档位；gate 值守 CPU 重画同理
  ——静帧内容等价，动画中帧判卷走 shot-gl GPU 真帧）。判卷待：用户
  真机 C 档（平移手感/Q 弹量级/跟手随动）。
