...
> **解析页视口化页面滚动（2026-09-20 在途，待 chain+判卷+终验）**：
> 用户拍板「卡弹小，然后里面的内容跟随截断，上下能滑动」——取代
...
> （>6 框既有），其余卡区垂直拖 = 页面滚动。落点：`ui/parser_page.rs`
> layout_vp（几何/钳制唯一源，涂装/命中/手势同一份）+ 状态核
> page_scroll（epoch 进 sig 自动重烘）；termview 涂装页缘裁剪带
> plumbing（paint_rect_ring_yclip/paint_divider_line_clip/文字
> _ex·_yclip 变体）；android_app 两产品路壳 inset + 手势分流
> ParserDrag{Session,Page}。钉：parser_chrome_spec 改钉（壳吃
> bottom_inset/内容吃栏带高双断言）+ parser_page_spec 视口化 4 考题
> （窗公式/整链平移/钳制/无键盘零行为差/状态机）。**判卷标准**：
> redroid 召键盘 → 页环弹小 → 卡区垂直拖环境卡滑出/回滑/到顶到底
> 钳制截屏。
