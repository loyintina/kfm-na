...
- **BAR-104：贴死交接闪变（2026-09-17，根因已定罪+已修，待 vc446 真机差分判卷）**：
  ...
  **vc450 判卷达标（2026-09-17）**：Upper 全序列差分
  ...
  配判「有没有」，逐帧判连续性是噪音。**新发现挂账（待用户
  拍板）**：下池选中行文字双画重影——光标层带选中行文字
  （BAR-089  opaque 芯修法）与 LowerRowsPan 行层全量行字在平移
  期走两条曲线（光标 250ms 缓动 vs 行层池高 glide），选中行
  文字同帧两处错位（vc449 a00 帧+用户录屏 f462/f494 实证）；
  根治方向 = 光标只画框（细环真半透明芯）文字只归行层，涉
  BAR-089 选中态视觉设计，C 档判卷归用户。
