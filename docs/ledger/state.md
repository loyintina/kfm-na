...
- **二十二修：池区帧饥饿根治件一件二（BAR-096，2026-09-16 凌晨，
  ...
  **提交 9c30693 三端齐推+vc434 双端热更**（真机 na-push-so 冒烟
  ...
  （合成率物理极限判例不变，与拆层无关）。**待判卷（真机帧数终验）**：
  需用户亮屏——凌晨实测 app 后台（foreground=false，am start 拉不起
  前台），帧数提升量（panc 帧数/panel-anim 仪表）未取到；终验口径 =
  切标签 + 点下池各一次，拉 `grep panc` 帧数与 `grep panel-anim` 推算
