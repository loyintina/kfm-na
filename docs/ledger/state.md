...
## 当前位置（2026-09-24 傍晚，IME 视口跟随，已修待判）

> **键盘弹收终端内容跟随动画**（用户拍板优化）：卡片收窄有动画但
> 终端内容瞬移——定罪 = field-reports 单条「视口平移 0↔18/20 行」
> 零中间步（sync_kb_shift 吃真实 inset 单跳，chrome 走过缝弹簧）。
> 修（a3ee444）：遮挡带改吃 chrome 采样值，GLES/softbuffer 两路
> 每帧 chrome 采样后同步 sync_kb_shift，apply_window_size 同尺；
> 只动视口不碰 grid/pty。契约落 ui-base.md 缝表。**仪器复验绿**：
> 「视口跟随」逐帧步进（11→19、16→19→20），中途截屏内容在中继位。
> 已修待判——等用户真机肉眼终验（注意：60Hz 下弹簧有效帧 ~3-4，
> 观感若仍嫌跳，升级路径 = 像素级平移，改动面大未走）。
>
