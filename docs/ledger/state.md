...
- **BAR-104：贴死交接差分机（2026-09-17 晨，仪器已落，根因待定罪）**：
  ...
  钉死——运动学全绿，病灶在交接像素）。仪器：panend-cap-req 点播
  → 平移末帧合成 panend-a.rgb + 首帧稳态 panend-b.rgb 双帧落盘，
  `scripts/na-panend-cap.sh` 一键拉回+差分报告。钉三枚+变异两咬
  （tests/gate_spec.rs）。**下一步：vc445 实机差分 → 定位不一致的
  涂装层（渐变锚/池高 ±1/文字 AA 嫌疑）→ 修 → 差分≈0 为达标线**。
