...
> 纪律：**每个里程碑必更新此页**。接手者冷启动顺序：AGENTS.md（机械注入
> 即第一读）→ 本页 → bugs.md → chain.sh 跑一遍。**用户报 bug 第一读 docs/active/排障手册.md**
...
> 报三症翻案重开（观测先行第 4 条活例）：①查看器开着没有全屏压暗
> ②关闭钮差点按不到（底距输入栏顶 30px）③下池光标框亮线透过卡面。
> **根因**：压暗画进配置槽 → 标签栏层/输入栏槽/下池光标层三 chrome
> 层全在压暗之上（症①③同根）；旧卡高 = 屏高−8 格 → 卡底压到栏带
> 边缘（症②）。
> **修复臂**：①压暗升组件 ModalVeil（ChromeSlot=16 全屏层，入
> comp_registry 带 Preview::VeilFade 语义化预览——压暗层是组件不是
> 背景色）：全幅 α150 黑直写 + comp modal/查看器卡涂装在卡位之上 +
> 卡区圆角剪影提 α=0xFF 后处理；z 序 = Over 之上恒最后画。②卡高
> 安全带：顶 4 格 + 底（输入栏带 220+2 格=292），卡体安全带内居中。
> ③层级归正：VeilSig 开合判据 veil_open(modal||viewer)，跳框仲裁
> 整段提到 in_input_bar 之前（框外点栏带收框路通）。CPU 兜底路无层
> 机制，跳框原位涂装留配置槽——行为零变化。
> **钉**：翻案 5 枚（modal_spec 卡高安全带/veil 开合判据/veil 入册
> 动效预览 + termview_spec veil 全幅直写 7 采样点/veil 卡在压暗之上）
> + 旧两钉改新约；变异四咬全中（摘 buf.fill/先卡后压暗/卡高回 4 格/
> veil_open 只认 modal，均即红复原绿）。
> **redroid 实拍判卷（硬证据 /tmp/bar163/fix/，redroid13:5556 错峰场
> 1260×2560 GL 帧）**：症①标签栏带/输入栏带/下池框亮线混合比 0.41
> ≡ α150 全屏压暗实锤（06）；症②关闭钮底缘→输入栏顶 141px ≈ 3.9 格
> （旧 30px；卡底 y=2267 = 2560−292 几何钉住，07 对照 07-base）；症③
> 卡面全扫无透线；框外点栏带收框路通（08）。**同族巡检**：comp modal
> 同修同收（16-compmodal.png 实测混合比 0.39-0.42）；parser 确认框
> 同形但轻微申报不修；文件树页查看器仲裁同款遮蔽归 x2 线在途件。
> 状态：已修待判（redroid 实拍达标，用户终验走静默判过）。
...
> 前缀反代 127.0.0.1:9041（只放 /api/ 面，502 诚实报错体）；agentd
> 新四端点（线内会话列表/点名会话 tail/信件列表/信正文，NNNN-*.jsonl
> 与 ASCII *.md 闸，非法 400 不存在 404）。设置页标签栏第三页
> 「会话池」（组件池兄弟页）：下池 = 路由表（线 + 信箱特殊路由），
> 上池 = 条目表（会话/信件），点条目 = 查看器跳框（comp modal 同层级
> 无画板版）看渲染对话（wire_render：tool 配对折叠 + usage 聚合账行）
...
> **真机自验全过（2026-09-26 晚）**：热更核上机后 na-touch 注入 +
> na-shot 实拍 + na-trace 遥测三轨——会话池页开/路由表出真数据
> （demo 线 + 信箱）/会话条目 241.6KB/查看器渲染对话（tool 折叠行
> 全在）/关闭钮收框/信件列表与 markdown 全文直读，B 档判卷闭卷。
...
> 86/118、缩进累加钳 160 CSS、三角 teal 20×22、左强调边 6px、底栏 110 三盒、
> 光标框五件）②数据面 na-server 两端点 `/api/fs/list`+`/api/fs/read`（纯函数
> 核在 na-protocol，**fs 走 spawn_blocking**——na-server 单线程不许连坐 WS；
> fail-closed 同 404 不透露存在性）③`ft_touch` 手势槽 + 抬手命中吃**屏代
> 快照**（BAR-145 判例）+ 文件行 = 查看器只读预览 ④**三处涂装同吃一份**
> （GLES 槽/值守倒帧/软渲染兜底），脏帧 sig 十维，页内动画进活性探针第六路
> ⑤comp_registry 两枚动效条目 + registry.md 入册。
> **判卷**：redroid 实拍对照原版截屏（行高/缩进/三角/抽屉/光标逐格）。
> 已声明偏差：原版行带横向渐变 → na 纯色（用户裁决）；容器 `wm size` 锁
> 2560 → 纵向 240px 硬差须补偿。
...
> （scripts/na-winstate-watch.sh 每 45s 远测）16:45:44 抓到发病几何
> `winTop=135`（同日 100+ 样本唯一 SICK，其余恒 0），与用户报障时刻
> 同期 [touch] 账 y=1653/1647 对上（nz 视觉中心 1787 − 窗顶 = 输入值）
> ——**输入空间 = 显示空间 − 窗顶**，三方同窗口闭链，前六度单侧账的
> 悬案结案。dispatchCount 恒 0 定死 Java 层拦截是死路（input queue
> 旁路）。修复两臂：①**主臂（已含在本次热更核）**：Rust 触摸入口
> 运行时补偿——`insets::drift_compensate` 纯函数 + `insets::query_win_top`
> 活读 decorView 窗顶（只碰标准框架类，现行装机 APK 直接生效），真触摸
> 入口起手采样一次、本手势同尺，通道八注入不补，[touch] 账尾附 `窗顶N`；
> ②**治本臂（Java 皮，需装新 APK）**：resume/focus 重贴沉浸式标志，
> 逼系统重算窗口 frame。判卷走静默判过（观测先行第 4 条）：用户无报障
> 即终验通过。**捕获器与退役闸照旧不动**（观察期至 2026-10-02）——
> 再抓 SICK 即凭 [touch] 账 `窗顶N` 判补偿生效与否。

## 当前位置（2026-09-26 午，BAR-157 反连腿认领循环陈尸憋死案已修上机实证通过）

> **62694 间歇全灭之谜定罪**（用户问「我们不是有 62633/62694 吗」
> 引出）：不是运营商 UDP 黑洞（10:05 抓包 62694 双向正常），不是
> 安全组（用户两条入方向 UDP 规则都在）——pcap 逐包对表：手机
> Initial 到网卡正常，服务器 20/23 次零回包，仅存回包 = 每 60s 一具
> 陈尸的 PTO 序列（60s = REV_IDLE_TIMEOUT）。病灶 = quinn Incoming
> 在应用 accept 前不回第一个包 × 旧认领循环串行 await 到终局——
> 一具陈尸（客户端 8s 弃连、服务器 60s idle 收尸）停摆 accept 60s，
> 弃尸 10.5s/具 > 收尸 60s/具 = 队列永远排不完。修 = accept 驱动
...
## 当前位置（2026-09-26 午，BAR-158 结案 + 增益/预热池判卷过关）

> **用户判卷原话**：「好了。感觉修复了」（BAR-158 追底态下滚闪烁，
> 方向闸三处同挂，热核 vc1790395273 上机遥测复验零负值切入）；
> 「那个两倍滚动和预热可以过关了」——下块 ①② 两项真机判卷销账，
> 预热池多通道并发播种/内存/分流正确性手感关通过。台上只剩
> BAR-145（tmux 列点击漂移）仪器常驻观察，一周无恙自动销案。

## 当前位置（2026-09-26 晨，预热池+滚动增益，BAR-155/156 用户结案）

> **BAR-155/156 结案**：用户早班原话「行为看起来是被修复了，没有
> 复制，正常滚动」——账本两行结案；BAR-145 补六度证言（熄屏钥匙
> 失效，「整个屏幕被移动了一行」，监控继续，仪器 APK 待 Termux 链路
> 部署）。
> **① 滚动 2 倍增益（用户拍板「移 10px 滚 20px」）**：
> `scroll.rs` `DRAG_GAIN = 2.0` 常量，`moved`/`moved_px` 统一乘——
> 拖动与甩尾初速同倍，slop 门不动。scroll_spec 17 钉全绿，变异
> 实咬（改 1.0 → 8 钉红）。契约 §3.2。
> **② 全会话预热池（用户模型定案：「打开 na 就把所有 tmux 窗口后台
> 加载好，切窗和滚动理论上不该卡」）**：旧制切会话/attach 即
> `ctrl_teardown` 拆通道 = 新会话首滚走 v3 抓拍那一卡的病灶本体。
> 改 = App 单通道四件 → `warm_pool: HashMap<String, WarmSess>` +
> `warm_names`；池扩缩唯一凭据 = 服务器真表 List（List 到达喂
> warm_names），容量帽 8 + 当前会话兜底；WarmSess 壳携
> ctrl:Option/feed/canvas:Option/pending/build/播种账（判负拆通道
> 留壳退避）；切会话/attach 只 park 画布还池**不拆通道**，切回零
> 等待；resize 全池 invalidate 通道不拆；对账档当前 5s/停放 30s。
> 契约 §3.5.1「进出」「预热池」条。**运行时行为只过编译，真机判卷
> 待用户手感**（多通道并发播种/内存/分流正确性）。

## 当前位置（2026-09-25 午后③，外置视口 v4 tmux -C 控制模式推流画布）

> **缘起**：v3 增量合并（250ms/2-4Hz）上机后用户质问「是不是便宜
> 渲染模式」——如实承认后用户拍板「直接换真方案」。v4 = 模型正身：
> **隐藏画布在后台随字节流即时生长，视口只是画布上的平移窗**；
> v3 轮询快照全链降级为保底（推流活着 = browse_fire_capture/250ms
> 刷新/温热快照全链熄火；推流判负 = 自动接管）。
> **力学**（tmux 3.4 逐条实证）：tmux -C attach-session -f
> ignore-size 常驻控制通道（第二条 ws，复用 ws_spawner，服务端零
> 改动）→ %output 三位八进制转义行即时推字节；带内播种双块
> （KFMHDR 头行 + capture 全史）与 %output 严格不交错 = 零丢失零
> 重复对齐；Canvas = Term + 常驻 Processor（转义跨块截半的拼回
...
> pty 实录原始流定罪三病灶：①attach-session 自己的空回应块被当
> 播种头块判负；②头块 %end 被当 capture 关（连环病灶）；③**pty
> 流 \r 尾剥除留在壳层**——parse_ctrl_line 剥分类不剥原文，
> parse_seed_header 的 pane 段 "3\r" 数字解析失败 100% 判负
> （ff57cc7 存证版报表铁证：「首行=KFMHDR 2898 10000 5 50 %3」
> 完全合法却判负）。修 = 相位机抽 ctrl_feed.rs 纯逻辑（六相位对
> 块序列），行装配/剥 \r 收编进 feed_bytes 唯一入口，android_app
> 删 ctrl_buf 改 ctrl_exec 动作薄壳；ctrl_feed_spec 八钉（fixture
> 全真流形态 \r\n）+ 变异三咬全绿。教训入账：**协议状态机不许落
> 在 host 不可测的 cfg 壳里，先拿对端原始流 fixture 钉块序；
> 字节→行→事件每道加工同属状态机，在哪层做就在哪层钉**。
> 判卷点：kimicode 输出中滚到中部，内容**连续即时**流动且阅读位
> 钉死；触底回 live 后再进零等待（画布后台续喂）；断网/杀会话
> v3 轮询无感接管。
> **BAR-156（2026-09-25 晚，动态行静态复制已修）**：用户实报
> 「上滚后动态行在动，上面还有一条它的静态复制；追底正常」。
> 服务器实录定罪：capture-pane 恒发全屏含尾空行，播种文本尾 ≠
> 真实游标（头行 cursor=(12,10)，文本尾在 23 行）——不归位则
> 续喂 \r\x1b[K 帧落屏底、旧帧留静态复制。修 = Build 携头行游标，
> Canvas::build 喂完补 CUP 归位。钉 spec_bar156 + 变异实咬全绿。
> 设计契约 docs/active/设置页.md §3.5.1。

## 当前位置（2026-09-25 午后②，外置视口 v3 immutable-history 增量合并刷新）

> **缘起**：用户实报 kimicode 输出时视口滚到中部，浏览态更新太慢
> （v2 是 1.5s 节流全量重抓），并质问是否「便宜渲染模式」。
> **v3 模型**：历史不可变——增量臂只小抓当前屏（rows 行级，
> `capture-pane -p` 无 `-S -`，尾标同轨验收），抓取线程内
> merge_capture 合并出全量（旧文前 hist_old+k 行 + 新屏 rows 行，
> k=hist_new-hist_old）；判负四路（k<0 清史/k>rows 超一屏滚动/
> 行数对账不符/hist 撞 history_limit 顶）→ browse_last 清账，
> 下回全量自愈。节奏 1500ms→250ms + 在途不叠（exec 往返自限
> ≈2-4Hz）。清账点：resize/switch_session/parser_attach 齐。
> 判卷点：浏览态滚到中部时内容以 2-4Hz 持续刷新（原来 1.5s 一顿）。
> 钉 tmux_ctl_spec 28 绿（增量合并钉+变异双咬：摘 k>rows 闸→红、
> 切片上界+1→红→复原绿）。
> 收尾：chain 全绿 → 提交推双远端 → 编核热更 → 用户重启 na 验收。
> 终局方向（已向用户交底，未立项）：tmux -C 控制模式 %output 推送
> 或字节续喂+对账自愈；v3 是 2-4Hz 精确刷新的务实逼近。

## 当前位置（2026-09-25 午后，BAR-154 外置视口 v2 三病灶修复，待编核热更+用户终验）

> **BAR-154 三病灶遥测定罪已修**（用户实报「拖动卡一下/滚动出界
> 一行一行瞬消失/kimicode 输出时上滚页面静止」；bugs.md 行全档）：
> ①锚方向反了——swap_browse 保 offset 原值 = 视口追新下移（遥测
> 实录 offset=37→37→27 里程在涨），修 = swap_browse_term 锚定内容
> 守恒（offset 补偿底部追加行数，钉改补偿语义+变异实咬）；
> ②同次触摸触底后重返浏览态（遥测实录 触底回 live→手指未抬→
> 切入(首抓) -143px），修 = browse_suppress 抑制闸（抬手/切会话
> 解锁）；③266KB 快照解析占 UI 线程，修 = build_browse_term 挪抓取
> 后台线程（剥尾标+解析同线程，排水只换壳）。
> 判卷点：拖动零等待切入、浏览期刷新阅读位钉死、触底后同次拖动
> 不重返、全程无解析卡顿。
>
> **v2 后台模型本体**（设计契约 docs/active/设置页.md §3.5）：①零感知切入——拖动起手用温热快照即进浏览态
> （闲时/抬手/退场后台抓好的），抓取只作后台刷新，无 0.3s 等待；
> ②浏览期动态播放——浏览态非静态：1.5s 节流 + feed_seq 代际比对
> 有新活动才重抓，swap_browse_term 原地刷新，视口锚定内容守恒
> （阅读位钉死，BAR-154 修正补偿语义）；③触底自动回 live——display_offset=0 且零头清零
> 即退场，不靠击键（击键退场保留作保底）。判卷点 = 此三条逐条对上。
> 钉 termview_spec 浏览态五钉全绿（含 swap 锚守恒/feed_seq 活动判据，
> 变异三咬全中）+ tmux_ctl_spec cmd_capture 标记验收。
> **BAR-153 修复已随本批**（cb153cf 已推）：capture-pane 目标改
> `'=name:'` 精确会话+活动窗格（裸 `=name` 在 pane 目标不成立，
> tmux 3.4 实证「can't find pane」页面消失案），KFM_CAP_OK 尾标
> 验收闸——报错文本永许不进浏览态。
> 收尾：chain 全绿 → 提交推双远端 → 编核热更 → 用户重启 na 验收。
>
> **BAR-145 仪器 APK（28630bd）已送机待装**：用户 2026-09-25 午实报
> 暂时复现不出——仪器包留机，再现时按复现钥匙（熄屏→解锁→进 na→
> 点 nz）走一轮即取证（motion-java.log + window-state 通道十四）。
> 观察期退役闸照旧 10-02。
>
## 当前位置（2026-09-25 晨，BAR-145 终局定罪：显示无罪，输入坐标偏移）

> **BAR-145 真根定罪**（bugs.md 行全档）：用户发现稳定复现钥匙
> （熄屏→解锁→进 na 必现 3/3）→ 三方对表当庭翻案——系统截屏
> （02:30:28 发病窗口实拍）与自动 na-shot GL 帧均显示行位完全正确，
> 而 [touch] 账显示 na 收到的触摸 y 比物理点按系统性偏小 ~130px
> （恰为状态栏高度）。**显示/几何/渲染全链无罪；病灶在 Android
...
> **BAR-145 已修待判**（bugs.md 行全档）：力学根 = 底锚槽+动态卡高
> × 名单翻动（9-23 夜遥测实证 4↔5↔8 横跳；用户「换 IP→重连风暴」
> 推测实证成立）。修两臂：①卡高容量化（行/分隔线/按钮钉死，增减
> 只动列表区底部空位；容量吃无键盘可视底，BAR-119 保底两行旧修约
> 废止升级为压力全脱钩）②命中吃屏代快照（baked_snap——点中的就是
> 看到的）。钉 spec_bar145×3 + spec_bar119 重写，变异三咬全中。
> **仪器观察期至 2026-10-02**：退役闸钉转红强制裁决——无再现拆仪器
> 结案，有再现凭屏代账续修（用户拍板「过一周没事自己取消或让我们
> 知道」）。

## 当前位置（2026-09-24 晚，像素级滚动 v1 落地，待用户终验）

> **像素级滚动 v1 已实现待判**（设计契约 docs/active/设置页.md §三）：
> 分数视口第二层——键盘 kb_frac 之后，手指滚动零头也进分数视口。
> ①scroll.rs moved_px（同 slop 门不取整，越阈第一笔 slop 段不计入）；
> ②TermView scroll_px（零头累计满行提交 display_offset、两端钳制、
> 拨关清亚行态）+ 渲染双路径（softbuffer/GPU）顶缘多收 line=-1 /
> 底缘多收 line=screen_lines；③GpuCell.py i64 化；④GLES 合成期
> 实例平移 dy - kb_frac + scroll_frac + present_frame scissor 带
> (顶带内缘, 内容底沿)；⑤触摸分流（pixel_lane 一次触摸内不换道；
> tmux 全屏鼠标上报照旧 SGR 滚轮行级）+ 启动读回；⑥设置页下池
> 第二行「终端设置/滚动行为」→ 上池「像素级滚动」下拉（关=行级
> 保底·默认 / 开=像素级实验），写盘 terminal.json pixelScroll。
> 考题 scroll_spec +2 / termview_spec +3 / settings_spec +1，
> 变异三咬全中（多收行删除/贴底钳制删除/moved_px 取整回潮——
> 均即红）。**旧行级保底开关关 = 零行为**，渲染收集与旧版逐格
> 等价（spec_像素滚动_关时渲染零差异钉住）。
>
> **上一批双结案**（用户终验「可以，效果不错」）：①像素级键盘
> 平移（1ac2ab7）；②BAR-152 弹簧欠阻尼→临界阻尼（3b87a73）。
...
> 晨间用户报打开 na 连接不上。定罪链——app（pid 13232）全程活着，
> logcat 心跳连续；field-reports 05:54 断更 = QUIC 腿死于网络切换；
> 重拉的新腿挂在 `connect().await`（UDP 黑洞里 quinn 握手要等 4h
> idle 上限），死信不发/本地口不绑/e2e 探活块（`if port_open`）整段
> 跳过——看门狗在 Starting 里空转 1 小时。修 = 握手 8s 速败 +
> 看门狗 Starting 超时（12s 宽限）主动定罪，跳闸账满 3 降级 ssh
> 兜底。钉 spec_bar146 真值表+变异实咬。**同轮自伤**：redroid 考场
> na（BAR-145 实验残留）一直没关，多实例并发 spam 伴生 ssh（每 15s
> 一条）挤占 9022——已 force-stop；redroid 实验完必须随手收场。
> 待用户终验：热更核上机后「打开 na」恢复 + 僵尸不再无限空转。
> **同轮 BAR-147 伴生赛跑根修**：伴生死亡块此前只做 child=None
> 跳过通用释放+退避块，零间隔重拉撞 9022 尸体活锁（logcat/auth.log
> 双侧实录）。修 = 死亡块全量审理 + 重生段前封锁闸 + 零间隔禁令。
> 钉 spec_bar147 + 变异双咬。契约：每条死亡路径都要过同一个
> 「释放+退避」判决，不许有抄近道到 spawn 的分支。
> **同轮 BAR-145 挂起（用户裁决）**：tmux 会话框命中下移案次日
> 网络恢复后实测正常，疑似只在极端网络抖动窗口出现——证据与
> 复现钥匙已钉 bugs.md BAR-145 行，再发按行内第一动作走。
>
> **解析页服务段退役改通道段（2026-09-24 用户裁决）**：「服务外部
> 借用」段实际没用，取消，换更实际的——①9021/9022/62633/62694
> 四口连接状况字段行；②调试选项：左半 [跳闸/投 QUIC]（一键降级
> ssh / 再投一票，裁决归 svc_card::toggle_verdict）+ 右半 [重启]。
...
> RFC 4231 标准向量判卷；常量时间比对；验签不过弃流+按源 IP 连败满 5
> 封 10 分钟）。考题 `spec_auth_*` 六钉全绿（对钥匙 echo 全还/错钥匙零
> 字节）。②**na-server 开腿即强制双证**：`{前缀}.psk` 首跑生成 0600，
...
> **待用户裁决**：§七问题 1——QUIC 走 UDP 必须绑公网新口（已拍双口：62633 正连 / 62694 反连），
...
> **BAR-133 预防式释放反连口（2026-09-22 午后，用户「突然掉线，等了三个提示才接回，十几秒」）**：真因 = **手机移动网换 IP**（实录 .41→.112→.47，同日内 .44→.45）——换小区/APN 时运营商丢所有既有 TCP，长连接必死；服务器 auth.log 无优雅断开记录（中间设备 RST）可与证。11 秒里有 5~6 秒是纯浪费：重拉撞上一轮残留反连口 + 自愈排在第二次失败之后。修 = 死前转发绑过就**预防式释放**（那条服务器会话此刻已无主），预期恢复 4~6s。**边界要讲清**：移动网换 IP 这一下防不了（TCP 层面就是断），na 能做的是「接回来的速度」与「会话不丢（远端 tmux）」。
...
> 且 `nasup 状态` 仍 1~3s 一跳（多实例仍在）。病机：单实例闸只挡新实例，
> 旧核实例继续抢反连口刷状态，**而每次重启又养出一个新实例**（越重启越乱）。
> 修 = 拿锁后、任何子系统起跑前扫 /proc 清同 uid 的其它 na 进程（判据三严 +
> SIGKILL；自测实咬：判据在宿主会误杀测试外壳 → 清场改**安卓专属**双闸）。
> 待热更上机看 `清残留实例 N 个` 与心跳单序列。
...
> **夜班（2026-09-22 01:43 窗口）**：①全量 chain 绿（服务器窗口，ionice/nice 双甲，
> 三次跑：头两次红在自测题自身的判据上——见下）；
> ②工作树当晚已全部提交三推（本轮无遗留）；③候补重载任务 = **BAR-127 转修**
> 落地：`src/singleton.rs` flock 单实例闸（进程死内核自动释放、第二实例
> 遗言指名持有者 pid 后退出、闸在任何子系统起跑之前）——这是今晚「反复
> 连接反复断开」最后一块根（多实例互抢反连口 + 多套看门狗刷状态）；钉
> `spec_单实例_第二把必败_松手必胜`（第二把必败/松手可再抢/子进程死后锁
> 自解）。**待手机隧道恢复后验证**：多实例残留清空 → 日志应回到「一分钟
> 一次状态迁移」（此前 1~3 秒一次）。④提交 `e722431`（三推：gitee/github 成功，
> **phone 远端失败**——Termux 8022 仍在冻结，等它回来补推）；手机侧暂不可达
> （9022 抖动中），本轮未能热更，手机仍跑**常驻模式落地前**的核（常驻已在
> 服务器侧生效：`kfm-na-server.service` active+enabled）。**夜班自省一记**：
> 单实例题的「同进程 drop→再抢」在 workspace 并行负载下不稳，头两次 chain
> 就红在这一条自命题上——已按「诚实边界」处理（该路径不设为判据，真语义由
> 子进程死题判卷），并把边界写进考题注释。

> **na-server 常驻模式（2026-09-21 夜，用户拍板「服务常驻在服务器，但它
> 依然是 na 的触手，na 负责监控它的状态」）**：unit 内容**随 na 走**
> （`unit_content` 单一源内嵌，只绑回环/永不自退/`Restart=always` 随机器
> 自启），na 每次连接经 SSH exec 幂等装/更新 + `enable --now`（在跑的不
...
> 15s 一次新 ssh 登录（连续源口 12142..12149）= na-server ensure 探针
> cadence；而 na 报告里同一秒多次 `nasup 状态` 迁移。**探针没病，是它的
> 在途相（Checking）被发布成客户端状态**——服务卡状态词如实显示「确认中」
> → 15s 一跳，被读成反复重连（会话其实一次没断）。修 = `publish_state`
> 在途相不发布，只发布结果相；初始相改 Checking。**今晚断连三案合计**：
> BAR-128（stderr 仪器 + 抖动退避 + 失败上报限流）/ BAR-129（反连口自愈
> 释放）/ BAR-130（在途相不跳出卡面）——从「猜」到「读」，从「等 90s
> 收割」到「主动释放」，从「探针节拍=卡面节拍」到「只有结果才翻字」。
> **待观察**：真机下一轮抖动读 stderr 真因（撞口已自愈；若是移动网 IP
> 切换/Android 冻结导致 TCP 断，那是物理层，只能做到「秒级恢复 + 会话
> 不丢（tmux 远端在）」）——已实测手机公网 IP 在两次查验间由
> 39.144.203.44 变 .45（移动网换点）。

> **BAR-129 反连口自愈（2026-09-21 夜，用户二轮「回到 na 就反复掉」）**：
> 机制闭环——退后台/灭屏/被杀时，服务端 -R 会话在 ClientAlive 收割窗
> （90s）内仍占本设备反连口（实测 9022 监听者 pid 每几分钟换一个 =
> 每轮重拉的痕迹），重拉 ssh 撞口 → ExitOnForwardFailure → 秒级 255 →
> 用户看到 1~9 分钟「反复连接」。修 = **不等收割，主动释放**：仅在 ssh
> 死因含「remote port forwarding failed + 本设备口号」时请服务器收掉占口
> 的 sshd（严判防杀活会话；只杀 comm=sshd），释放落地前首轮退避 ≥4s。
> 契约：每设备 10 口段的口即设备身份，占本设备口者必是自己的遗留会话。
> 手机已热更 677892c（其后 0 次 255 实录待观察）。

> **BAR-128 隧道反复重拉（2026-09-21 夜，用户问「为什么反复连接反复断开？kfmv4
> 网页随时能连，na 本地端不是该更稳？」）**：现场实录定罪——`[tunnel] ssh
> 进程退出（255）第 1~20 次退避重拉`成串（12:42-12:51 九分钟爬 20 次、
> 13:20-13:22 三分钟 8 次），spawn 后 1s 内即死；服务器侧查得两条到手机的
> ESTAB 连接 **Send-Q 268KB/157KB 长期积压**（服务端发不出去 = 手机侧 ssh
> 被冻结/网络切换不读），sshd 加固已生效、9022 非僵尸——BAR-126 的僵尸病
> 换了个方向。修两处：①**stderr 上管道**（BAR-126 挂账「ssh 死因被灭口」
> 落地）——退出行现落真因；②**退避只认「活了多久」**（`next_attempts`：
> 活 ≥30s 才回 1，短命娃计数续涨 → 2/5/10/30s 爬）——原先「一见 Up 就
> 清零」在抖动网络下=每 2s 重拉的热循环；③svchealth 失败上报限流 60s/面。
> **架构观察（答用户之问）**：kfmv4 网页稳，是因为**会话住在常驻的服务端
> 守护进程里**（systemd 常年不睡），浏览器只是可弃的视图；na 的会话也在
> 服务端（tmux），但**传输（ssh 隧道）住在 Android 应用进程里**——系统
> 随时冻结/杀它，一冻就断流、一断就重拉，于是「反复连接」出现在传输层，
> 会话本身其实没丢（tmux 还在）。真因待下一轮实录（仪器已就位，见
> bugs.md BAR-128）。
> **同期提醒**：na 全退后台时不再攒环境卡历史（前台即抢的边界）；回前台
> 即续摊、落盘件也还在。

> **环境卡柱轨 v3（2026-09-21 夜，用户真机二轮四条）**：①**默认铺满
> 50%**（「一开始就生成一个默认 50% 的铺满的版本，然后后续从右侧慢慢
> 出现」）：槽位右对齐（`sys_hist::slot_of`——最新占样本最右槽），左侧
> 未铺满的槽 = 50% 高的中性青低 α **占位柱**（不是数据，与三判色档一眼
> 可分），真实样本每拍从右进来把占位逐根顶出左缘。②**100% 线**：轨底
> 以上 30px 的 1px 白 α96 线压柱横贯轨宽（刻度必须看得见），超 100%
> 的读数越线不越轨（轨高 36 = 线 30 + 余量 6）。③**进程行删除**（「进程
> 数字删除吧」；procs 仍在契约面）。④**没数据的行不做**（「如果切换到
> 手机，没有的条就不做吧」）——行集数据定（`sys_card::rows_of`：值不是
> 「—」才画行），卡高随行集（`card_h`；手机相只剩三轨、托管相只剩卡头），
> `parser_chain::heights` 添 `sys_h` 参（卡高不再是常量；排布器不读全局
> 快照，考题钉固定夹具）。**核对 kfmv4 原版后的诚实结论**：kfmv4
> 那版**没有** 50% 占位（它靠服务端 5s 采样 + 环形 40 点 + 落盘
> `~/.kfmv4/ledger/sys-metrics.json`，窗口常年是满的；短历史时也是左对齐
> 少数柱）+ 客户端恒渲染 17 根、稳态 translateX(−5px)、seq 驱动（值不变
> 照滑）、逐样本判色、柱高 = min(1, v/100)×16px。占位铺满与 100% 线是
> 本轮用户新定的规格。

> **环境卡柱轨 v2（2026-09-21 晚，用户真机回炉三条）**：①**别做从左长，
> 默认就铺开**——历史账改成「前台即抢 + 落盘续摊」：拍条件 = 解析页
> 可见 或 应用在前台（退后台零流量；health 面仍只认页可见），账每 15 拍
> 落 `{files}/sys-hist.txt`（文本行制，编码/解码 = sys_hist A 档纯函数），
> 进程重启从盘上续摊；**对象轴两相各一本账**（服务器/本地互不清带，
> 切环境各自续摊；Server 账按 nasup 目标串归属，换服务器即清）。
> ②**负载判色**（用户「这个服务器自己是 4 核的」）——`cores` 进
> na-sys/na-server 契约（采集侧 available_parallelism，各机器自报），
> 负载占比 = l1/核数 → 并入三档判色（kfmv4 同规）；核数未知（旧版后端）
> 回退窗峰归一 + 中性档。③**ensure 脚本加「源码新即重建」**——na-server
> 是契约一端，只判「二进制在不在」会让契约变更默默不生效（本机实测：
> 老进程照旧下发旧契约）；在跑的老进程不掐（别人的会话挂它上面），
> 等 idle 退出或下次拉起换新。**redroid 判卷**：`cores:4` 下发实拍；
> 负载轨红档（143%，机器正跑我的编译）；重开进程 + 立刻开页 = 四轨
> **满宽铺开**（97..574，无从左长）；前台页关时 seq 60s +30（= 2s/拍，
> 前台即抢成立）。**待真机终验**：手机环境（本地相）柱轨——Android 拒
> /proc/loadavg 与 /proc/uptime，本地相的负载轨会**无柱**、在线显示
> 「—」（诚实显形；mem/swap/disk 照常）。④**进程行自证**（用户问
> 「进程的那个数字是什么意思」）：`2/123` → `2 运行 / 123 总`（内核
> 口径 = 可运行调度实体 / 系统调度实体总数，含线程）。⑤**本机体征每拍
> 都采**（与当前相无关地续摊本地账——na 直读零网络零成本，切到手机
> 环境那本账已是满窗；托管相空转支排在它之前，防 200ms 空转把本地账
> 灌成 200ms 采样）。

> **环境卡重做（2026-09-21 晚，用户拍板「复刻 kfmv4 中央面板的动态负载
> 统计表，整个单竖列，不要并排」）**：卡形 = 卡头「环境 · 对象词」→
> 四轨（负载/内存/交换/磁盘：文字行 [标签左锚 + 值右锚，值随判色档上色]
> + 行下滚动柱轨）→ 两尾部行（进程/在线——计数与时长不是占比，无柱）。
> 脑 = 新 `src/sys_hist.rs`（A 档：环形账 192 拍 ≈6.4 分钟、判色档
> >85 红 / ≥70 琥珀 / 其余绿、柱高归一、匀速滑入算术、`band_place`
> 放置纯函数）+ svc_health 轮询器同拍追（值不变照追 = 时钟驱动；翻相
> 清账）。皮 = 新槽 `ChromeSlot::SysBars`：**滑动全在合成期**——层内容
> 只在采样换代时一烘（卡内芯渐变逐像素重建 + 柱稳态位），滑入位移 =
> 源 uv 窗口原点（dest = 轨矩形 ⇒ kfmv4 overflow:hidden 等价，零
> scissor 零漏墨；帧泵 1px 粒度 ≈4.5 帧/s）。宪法：theme.md §2.4 示警
> 色档（新立；中性柱固定青 `0x2E9FD0`——实拍定罪「黄调 accent 下中性柱
> 与琥珀柱撞色」）、§六之十 体征柱轨、§七 标定行；解析页.md §四附则 +
> §六 第 11 步；组件池入册「体征柱轨」（Preview::SysBars 小画板真涂装）。
> 考题：新 `tests/sys_hist_spec.rs` 18 钉（环形/滑入钳位/阈值边界/柱高
> 归一/样本合成/uv 尺寸语义/纵向裁剪）+ sys_card_spec 单竖列几何钉重写，
> 变异八枚全咬；chain-phone 全绿。已提交三推 `fe40816`。**redroid 判卷**：四轨柱带与 CPU 路
> 逐行对齐（负载 299-324 中性青 / 内存 440-450 绿 / 交换 575-576 绿 /
> 磁盘 678-702 琥珀）、相位日志 boff 0→9 每拍爬升、20s 内柱数 9→19 且
> 尾柱 x+88、键盘弹起页面弹小后柱带不越卡底。**首版真 bug 一记**：图层
> shader 的 `a_uv` 是 (原点, **尺寸**)，首版传远角 = 源窗纵段翻倍（柱带
> 错位越轨），redroid 截屏定罪后改尺寸语义并钉进考题；另一记 =
> vbottom_inset 漏加栏带（与页烘焙裁剪带错开），当场改正。
> **观测口径**：柱轨样本只在解析页可见时积累（不可见不轮纪律）——开页
> 头 2 分钟柱随采样生长，之后恒满窗滚动；应用重启即清账。
> **待真机终验**：柱轨观感/示警色档配色/滑入顺滑度（redroid 帧率不代表
> 真机）。**挂账**：卡链滚动裁剪带场景（scroll_max>0）本班造不出（卡高
> 828 < 区窗），待后续验证。

> **三案结案（2026-09-21 午，用户肉眼终验「效果是好的」）**：
> BAR-123（键盘弹起 Ctrl-] 失灵）/ BAR-124（切换残影）/ BAR-125
> （切回本地残屏+滑动乱码）全部结案。本班另落地：晨班会形态审计
> （BAR-114 升代码守卫 PARADIGM v2、BAR-116 挂账理由）。**9022
> 自持反连（用户问「9022 能不能推？我们自己的推送路径？」，已
> 落地）**：9022 设计上就是「反连直达 NA sshd」（与 8024 同
> sshd 双入口），但 v1 把 -R 划给 Termux 维护 → Termux 冻结它就
> 不存在。正解 = -R 并进 na 自持隧道：tunnel.rs forward_args 追加
> `-R 127.0.0.1:{remote_port}:127.0.0.1:8024`（显式绑回环，不新增
> 公网暴露面），看门狗双腿（BAR-117）维护——na 活着反连就在，
> 推送不再依赖 Termux。钉 tests/tunnel_spec.rs spec_转发参数_反连段
> （变异两咬：摘 -R 段 / 裸口打头皆红）。**端到端已验证**：手机
> 热更后服务器 9022 监听就位（na 的 ssh 持有），`ssh -p 9022` 直达
> NA sshd（u0_a376）；na-tunnel 默认口切 9022（8024 留 Termux 代维
> 备援别名 na旧路）。**redroid 避撞口**：同核同配置会抢 9022 →
> redroid servers.json remotePort 改 9122（每设备 10 口段规则），
> 双口共存无争（注：redroid 沙箱内没起 sshd，9122 通道空挂，
> 观测本就走 adb 传输，无害）。**挂账**：运维脚本舰队
> （na-push-so/na-restart/na-shot 等十余个）仍硬编码 8024，迁移
> 9022 是跟进项——9022 稳定跑一段后再统一切。
>
> **BAR-125 每会话模式快照 + 切换清屏（2026-09-21 早，实现落地待
> 提交）**：用户真机报「切回本地后屏幕还是服务器 tmux 内容 + 上下
> 滑跳手势操作符乱码」。定罪：乱码 = SGR 鼠标滚轮序列——tmux 鼠标
> 上报残留在共享仿真器 mode（切换不换模式），本地裸 shell 滑动被
> 翻成滚轮事件发 pty 原样回显；屏幕残留 = 单网格无快照 + 裸 shell
> 无重画协议（BAR-124 边界同根）。修 = 切出存 mode bits、切入复位
> 受管模式+清屏（用户拍板「切换后直接自动清屏」）+恢复切入方快照。
> 新纯模块 src/sess_mode.rs + 钉 tests/sess_mode_spec.rs 六枚（变异
> 两咬全中）；遥测锚「模式快照」。已提交三推 96c0aa6；redroid 判绿
> （快照 bits 往返原样回账、切入本地清屏实拍）；手机热更 10:48
> 闭环（**新路径：服务器交叉编 arm64 核 + 8024 直推**——Termux
> sshd 休眠冻结一整天，8022 依赖被整条绕开；/tmp 权限坑与「败也推」
> 值守链缺陷各一记：日志改 home、编成才推硬闸）。redroid local
> shell 坏（CANNOT LINK）造不出乱码回显场，终验归真机。
>
> **BAR-124 切换即刷新（2026-09-21 早，实现落地待提交）**：用户真机
> 报「Ctrl-] 切换后页面是对端残影，切换应该刷新一下页面」。病灶 =
> 双会话共用单一终端网格、无每会话屏幕快照——静默待机 replay 为空
> 时切回全是上一个会话的画面；同尺寸 Resize 补发净变化为零、对端
> 收不到 SIGWINCH。修 = resize 抖动：切换先发 rows-1 抖动尺寸 +
> 武装归位账，主循环 120ms 后归位（间隔必须：conn.rs Resize 只留
> 最新值，背靠背被合并）；真 resize 到达即作废抖动账。钉 =
> tests/session_router_spec.rs `spec_bar124_抖动尺寸_减一行保底`
> （变异实咬）；遥测锚「切换强制重画: 抖动 resize」/「抖动 resize
> 归位」。**欠账挂账：每会话网格快照（raw shell 无重画协议的根治）
> 不在本案**。已提交三推 7e4f599，双端热更（手机 08:08 重启闭环
> + redroid）。**redroid 判绿**：logcat 设备时戳三轮抖动/归位逐对
> 干净（165~209ms）、tmux client pts/7 实捕 64x55 闪变归位（SIGWINCH
> 链端到端）、截屏无残影。**判卷盲区钉：field-reports.log 时间戳 =
> 接力到达时刻（重复/乱序皆 artifact），计时判读只信 logcat**。
> 等用户真机终验：Ctrl-] 切本地 tmux 再切回，页面应立即刷新正常。
>
...
> **两轴第 8 步：解析页三区 v3 放宽（2026-09-21 午后用户拍板，
> 工作区在途未提交）**：v2 真机观感回炉四条款——①右列 16 格定宽
> → 页全区 **1/2 比例制**（「还是占半屏合适」，`col_w()` 网格对齐，
> `COL_W_NUM/DEN` 单常量日后翻 1/3 只动它）；②tmux 卡钮带**竖排**
> 一钮一行全宽（「并排两个更窄」；btn_rows/btn_block_h 几何唯一源，
> 确认态按常态几何排位零跳变）；③右上区**钉顶钳高卡内滚**（「做
> 卡片的压缩，里面内容的滑动，不要做卡片的滑动」——slot_rect 右上
> 臂框钉区顶高钳区窗，右账位移进 link_card::layout_in scroll 参，
> content_clip 内芯带断墨/命中同源；scroll_max 数值与区滑时代逐值
> 相等，左区区滑不动）；④**光球横向避让**（「把 tmux 窗口让出来」
> ——parser_chain::orb_avoid_x 唯一源：球心上限 = 右列左缘 − 球
> 半径，超出 × 页滑入 progress 收敛，pressed 不避让，状态核 x 不改
> 写；paint_over 两路径 + gate 值守倒帧（仪器同尺）+ 起手命中四处
> 同吃一函数，GLES 调整件进 OverSig 滑入期逐帧重烘焙与拖球同档）。考题：parser_chain 7 钉 5 变异
> 全咬 / parser_page 钮竖排钉+半宽回潮咬 / link_card +2 钉+内容
> 忘平移咬。**判卷标准**：redroid 截屏 = 右列半屏、长值一行不折、
> 钮竖排；真机 = 键盘弹起右上卡框不滑内容内滚、解析页滑入光球
> 让出右列。遗留待裁：环境卡在左区只剩半屏宽后两列六字段长值
> 观感（用户未提，报告里提请裁）。

> **BAR-126：na 反复断连重试事故（2026-09-21，用户终验结案）**：
...
> /data，零网络零后端）。compose 添对象相参数：本地相词 = 注册表
> 静态词「本地」、后端相不掺和（Kfmv4 后端也出本机体征）；字段
> 映射抽 fields_of 两相共用唯一源。svc_health 轮询器对象轴分流：
> 本地相只轮 sys 直读（health 面本地相内容归④，服务器账按
> 闪断不清卡面纪律留到翻回/④接线）；翻相清账（服务器体征
> 不许带进本地相，互不清带）+ Kfmv4 相节拍复位保鲜（翻回
> na-server 即拍）。考题 +1（本地相词/字段/占位三钉），变异
> 分支反向咬住。**判卷标准**：真机 Ctrl-] 切本地 → 解析页环境
> 卡卡头「环境 · 本地」、六字段出本机真值（与手机 Termux
> 里 uptime/free/df 对表）。
...
> 截断，上下能滑动」——取代
> BAR-120 同线遮盖契约（壳与内容同吃栏带高）为**视口化契约**：
> ①壳吃 bottom_inset（键盘在场页环弹小到输入栏带以上，环底 = 视口
> 底）；②内容布局仍只吃栏带高（BAR-119 红线不动：只盖不重排，卡
> 全价）；③整链（tmux 卡→连接服务合并卡→环境卡）随 page_scroll
> 统一平移——逾视底归键盘/栏带遮盖，逾视顶归页缘纵裁剪带（环/
> 分隔线/字段行/文字同带断墨）。滚动窗 = max(0, 链底 − 可视底)，
> 无键盘恒 0 零行为差。手势仲裁：起手落会话框表带 = 表内滚动
> （>6 框既有），其余卡区垂直拖 = 页面滚动（ParserDrag 分流）。
> 落点：`ui/parser_page.rs` layout_vp（几何/钳制唯一源）+ 状态核
> page_scroll（epoch 进 sig 自动重烘）；termview 页缘裁剪带
> plumbing（paint_rect_ring_yclip/paint_divider_line_clip/文字
> _ex·_yclip 变体）；android_app 两产品路壳 inset + 手势分流。
> 钉：parser_chrome_spec 改钉 + parser_page_spec 视口化 4 考题；
> 变异 4 枚全咬（漏 clamp/窗公式 −1/漏平移元素/壳回潮）。
> **redroid 判卷（热核 md5 三方一致）**：①壳弹小——页环底缘角弧
> 实测 y≈1622 = 2560−702(键盘)−220(栏带)−16(边距)，截屏角弧在栏带
> 以上；②Page 滚动——环境卡起手 logcat「→Page滚动」，拖 500px 钳
> 305 环境卡全字段滑出；③顶缘断墨——卡链上滑后页环顶缘完整、
> 内容齐环内顶裁断；④回滑到顶钳制——结构线采样 0 差异归位；
> ⑤Session 分流——8 会话造表滚，框表带起手 logcat「→Session滚动」，
> 行滚卡静；⑥横向滑页仲裁无回归。
> **BAR-121 同线层级补丁（2026-09-20，用户肉眼终验通过结案）**：页环底缘线被卡链内容盖掉（「内容把卡片下沿盖住了」）。
> 定罪：visible_bottom 返回环底缘**线本身**（1622），内容裁剪带底
> 同吃 1622，内容可画到 y=1621 盖掉环底 3px 边线；redroid 判卷只扫
> 角弧区漏了中段（教训：框线判读必扫中段）。修 = visible_bottom 多减
> AI_PAGE_FRAME_W——滚动窗与裁剪带底同一份尺，改一处两路齐修；无键盘
> 零行为差不变。契约：**框压内容是层级律**，内容可视底必须让出页环
> 底缘厚度。改钉 spec_视口化_可视底公式 + 变异实咬。
> **下一刀已定**：本地相解析页三卡两相 + 环境卡本地相 collect("/data")
> 直读（用户 2026-09-20 拍板）。

> **环境卡 v1 落地（2026-09-20，待提交+判卷+终验）**：解析页第四张
> 二级卡（na-server.md §四）——「中央终端所在环境的自身体征」通用
> 面。新册：`crates/na-sys`（负载/内存/磁盘解析采集，na-server 与
> 客户端同一份，A 档 7 考题三变异）+ na-server `/api/na/sys` 面
> （live 对表 /proc//df 逐字段一致）+ `src/ui/sys_card.rs`（恒定高
> 几何，A 档 8 考题三变异）。svc_health 扩面：sys 与 health 同轮询
> 器同 2s 拍。壳接线：inset 六处常量扩席 + 卡区仲裁吞环境卡 +
> termview 涂装。组件池已登记。**判卷标准**：redroid 截屏四卡纵排 +
> 卡面与 `curl /api/na/sys` 对表 + kfmv4 托管态。
> **下一刀已定**：本地相解析页三卡两相 + 环境卡本地相 collect("/data")
> 直读（用户 2026-09-20 拍板）。

> **服务卡 v1 落地（2026-09-20，提交 435090a，redroid 判卷 4/4 绿 +
> 用户真机终验通过结案）**：解析页第三张二级卡（na-server.md §四）
> ——卡头「服务 · 状态词」+四字段行（后端/在线/会话/错误）+ 分隔线
> + 每会话一行，v1 纯展示。新册两本：`src/ui/svc_card.rs`（三源合成
> 文案 + 动态高几何，A 档 13 考题，变异三枚全咬）+ `src/svc_health.rs`
> （health 轮询器：页可见 2s 一拍不可见不轮，http1 打隧道本地口零新
> 依赖；Error 相保留旧数据闪断不清卡面）。redroid 判卷：三卡纵排截屏
> + 会话行与 curl health 逐字段对表 + kfmv4 托管态 + 恢复复验四绿。
> 上机即显形 BAR-118（idle_s 年龄冒充空闲，提交 87134db 修约真空闲，
> 用户问「闲 24s 是什么」牵出——字段名不副实不许活过第一个消费者）。
> **下一步已定**：①环境状态卡（na-sys crate + /api/na/sys + 第四张卡，
> 远程相先行）；②本地相解析页三卡两相（用户 2026-09-20 拍板）。

> **U+FFFD 增量解码已修 + 变异双验（2026-09-20，提交 9427d66）**：新 A 档
...
> **替换符字形补丁（2026-09-20，提交 47e66f8，用户肉眼终验通过结案）**：tofu 悬案结案后用户拍板
...
> **真机已切 na-server 后端（2026-09-20，用户终验通过结案）**：提交 9427d66
...
> 输出自我烘焙）。**用户肉眼终验通过（2026-09-20 午结案）**：真机
> 日常用（输入/重绘/tmux 切换）无异样，FFFD 菱形问号字形真机实屏
> 确认——本段随本次 docs 提交归档。
...
> 用户真机终验看点：断线时卡自动出、[重试][切本地] 命中、传输恢复
> 零敲键接回、连接卡两相文案。
>
...
> 30788），8024 闸门在 app 沙箱里 pgrep 永远看不见（沙箱盲区二次
> 实锤：8024=APK 内 sshd，8022=Termux sshd，探活必须两闸都过）。
> 退役工序（8022 进 Termux）：先杀看守脚本 23407 再杀娃 30788
> （顺序反了自愈回路会重绑）→ 9021 CLOSED → **app 看门狗 ~35s 内
...
> **redroid 判卷（9c32ee5）**：缺件相截屏——双卡纵排/卡头「连接 ·
> 未启动」灰档/四字段行（目标 —、本地口 127.0.0.1:9021、重拉 —、
> 错误「ssh 配置缺件」红色）/[重连] 钮全对；点重连 →
> 「起手→解析页卡区（618,1293)」+「连接卡点重连 → 下达false」
> （缺件相 TUNNEL_CMD 未注册 = false，诚实留痕不静默不崩）。
> 真机终验看点：自持在线/外部借用两相文案 + 重连真杀娃重拉。

> **L3 自持隧道·真机接管实拍（已闭环——终判绿，见 2026-09-20 头条；
> 半程记录留档）**：热更后
...
> （「·N窗/·他端」meta 撤，附着框名字带 ● 前缀）；②× = 跳框模态
> 确认（压暗层+居中卡+[确定关闭][取消]，点框外=取消）取代卡内确认带；
> ③↻ 刷新钮撤（列会话开页/操作后自动刷，手动冗余）、「+新窗」→
> 「新窗」；④BAR-114：组件池跳框被下池三级框光标行压盖——BAR-096
> 拆层盲区（光标层合成在配置槽之上，模态只画进槽画布），修 = 模态
> 在时光标层不画。钉：parser_page_spec 重写 16 题（两列几何绝对框宽
> 钉/跳框几何/确认态命中屏蔽/按钮映射）；变异两咬全中（忘减列距→
> 绝对框宽钉红——相对位钉曾被 COL_GAP==CARD_PAD_H 巧合蒙混，补钉后
> 实咬；跳框卡外退化为 None→红）。组件池「三级框行」条目已备注
> 两形态+最小 2 格高半格空隙+tmux 框行引用关系。

> **BAR-113 符号全域字体补丁（已修待判，待 redroid 实拍+用户真机终验）**：
> kimi 输出 ·（U+00B7）—（U+2014）✅（U+2705）✨（U+2728）tofu 目击。
> 借字从清单制升整段制：borrow 增「源字体已有码点跳过」守卫 +
> cells="auto"（EAW 定格宽）；Latin-1/通用标点/货币符号三段整借
> （DejaVu 双级捐体）+ emoji 29 个高频（Noto Emoji 黑白捐体，OFL，
> 构建期依赖不进库）。产物 1.51→1.53MB。钉 spec_字体_符号全域补丁
> （变异抽检实咬）。判卷 = redroid 静会话 echo 符号串 screencap 零
> tofu + 真机 kimi 会话实录。「完全补上」口径 = 使用域全覆盖；真彩色
> emoji 与单色像素管线不兼容不做；零宽/格式控制符不补（本不该有墨）。
> 附：BAR-040 考官前提过时（默认启动已 attach kfm-na tmux，首屏无
> local shell 横幅）——从 na-push-so.sh 冒烟名单摘除，考官头注留档。
...
- **夜班一行（2026-09-20 01:43 窗）**：服务器全量 chain ✅ 11/11
  （双甲）。夜班产出：连接/服务卡 v1（9c32ee5，redroid 判卷绿——
  缺件相截屏文案全对 + 重连命中「下达false」诚实留痕）+ 隧道接管
  实拍半程（让位语义绿；接管被 na-tunnel.sh 自愈回路抢占——它并未
  死透，pgrep 括号法误判；手机 ~15:49 UTC 全线掉线中断）+ redroid
  RecreationAttempt 恐慌环清场（force-stop）。手机通宵未回线
  （8024 watcher 在蹲）——待办：回线后真退役 na-tunnel.sh 判接管
  全序列 → A 断线状态卡 / C 静默死点治理。
