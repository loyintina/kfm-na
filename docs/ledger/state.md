...
- **设置页 v1a 四刀全落笔（2026-09-13，ccc8c54+377959c+本提交）**：第一卡
  「系统管理/服务器配置」。①解析核 src/settings.rs（A 档 13 题+变异三咬）；
  ②壳接线（切换键/默认会话/ConnConfig 走配置文件）；③三层目录 UI
  初版（377959c：实机拍出联动全通，但用户判「字体太小/上下池分工错/
  无三级框」推翻）；④**池卡模式重对齐**（本提交：修宪 §五 二版——
  下拉不与下池联动/池行 3 格圆角框/accent 描边选中/上池字段框行；
  cfg_page 重写 13 题+变异两咬；**下拉换选 = terminal.json
  defaultSession 写盘**生效）。设计档 docs/active/设置页.md §2.7 台账。
  **待办：实机判卷**（热更 + 闸门实拍，观感 C 档终审在用户）；
  字段编辑/+新增/三级全屏页/✓ 标记 = v1b。
