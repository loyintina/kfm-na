...
## 当前位置（2026-09-24 晚，IME 视口像素级平移，已修待判）

> **行级跟随被用户实机判「观感怪」→ 升级像素级平移**（用户拍板）。
> kb_shift 单位行→px：整数行部在收集期平移（collect/render 同尺），
> 像素零头部 softbuffer 在落笔坐标（draw_glyph clip_top 裁顶带）、
> GLES 在合成期实例平移（vpush.dy - kb_frac）+ 顶带 scissor
> （gles_present grid_clip_top）；零头>0 边界行多收补底缝。
> A 档考题 6 题绿 + 变异两发实咬（cursor_row-1 / 去边界加宽均被抓）。
> **仪器复验绿**（1ac2ab7 上机 pid 19622）：field-reports「视口跟随」
> 读数出亚行值（753/754/743/318/185px——行版只可能出格高整数倍），
> 弹收逐帧小步进（776→769→758→754→753），中途截屏内容在中继位。
> 契约落 ui-base.md 缝表键盘 inset 行；分数行 = 理想视口模型第一层，
> 后续泛化到手指滚动（scroll.rs pending_px 零头接同一视口单源）。
> 已修待判——等用户真机肉眼终验：弹收键盘内容亚行连续滑动，
> 顶缘无字探进卡环，底缘无黑缝。
>
> **BAR-152 弹簧过冲取消**（同日用户拍板「过冲效果取消，内容随动
> 后视觉疲劳」）：field-reports 实录定罪过冲（趋 754px 序列
> 720→776→769→758→754 回摆）。spring_pos 欠阻尼→临界阻尼
> （零过冲单调趋近，ω=24 贴死 ≈440ms）；钉 spec_bar152 双向三程
> 零过冲+单调，变异双咬。契约入 bugs.md：跟随类动画只许零过冲。
> 仪器复验判据：弹收序列单调趋目标、无越过回摆。
> **复验绿**（3b87a73 上机 pid 20547）：弹起 342→538→650→704→731
> →745→749→752→753→754 严格单调零过冲；收起 748→322→87→0 单调
> 归零——旧版 776→769→758 回摆形态绝迹。已修待判同待终验。
>
> 复用上轮信箱事：v2 写信规范生效，na 表态信 0004 已投（可读性
> 契约按评审三修正采纳），0001 收齐翻已回。
