...
//! 2026-09-12 三修立、四修标定、09-13 五修宽+色，用户拍板）。核心层
//! 纯逻辑 A 档，只产几何，不碰像素。
//!
//! kfmv4 `canvas-cursor.ts` 开口框复刻：左强调线 + 左上/左下圆角线宽
//! 沿弧渐变 + 顶线锚左 + 底线锚左，**无右边**；底色绿青半透明垫整个
//! 框体。颜色跟随本卡随机 accent 双色渐变（五修用户拍板，推翻 09-12
//! 固定纯蓝决策——与页环同一把 135° 渐变尺同源采样，涂装层兑现，
//! 本册不碰色）。
//!
//! **四修标定（用户实机判：kfmv4 像素直搬太细、左缘没跟二级卡对齐、
//! 圆角与全页不统一）**：线宽/圆角不按 kfmv4 像素（细1/粗3/R4）直搬，
//! 改 NA 装修框同尺——细线 3px（= 装修框细缘）、左粗 9px、R=12
//! （kfmv4 R:细线 = 4:1 比例 ×3）；左线画在**框内缘**（kfmv4 的
//! 1.65px 突出不移植——装修框左缘都在框内，光标同规），标签行原点
//! x=61 与双池左框逐像素对齐（tab_bar 兑现，本册只钉线宽几何）。
//! **五修标定**：左粗 9→8px（用户实机判粗 1px，不再钉粗左缘同尺）。
//!
...
/// 左强调线宽 px（五修实机标定：9px 用户判粗 1px → 8px；不再钉死
/// 装修框粗左缘 3×W 同尺，光标左线宽度以实机观感为准，宪法 §四）
pub const EMPHASIS_W: i64 = 8;
