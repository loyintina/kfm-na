...
| down_card 断线状态卡 | `ui/down_card.rs`（几何命中纯逻辑；触发相 = session_over） | `termview::paint_down_card`（终卡槽烘焙末尾，与齿轮同槽同隐现） | touch-in（android_app Started 命中分流，session_over 才可达）+ kick_reconnect/switch_session 双动作 | down_card_spec 4 | 终端页族 TERM_FRAME_C1/C2 | docs/ledger/state.md A 断线治理条 |
