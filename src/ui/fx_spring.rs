...
/// 单核红线）。四道缝 + Q 弹形变弹簧共用一只钟（2026-09-04 键盘 inset
/// 缝入册、09-10 配置面板 X 缝入册、09-11 文件树面板 X 缝入册、09-12
/// viewport_push 形变弹簧入册：同窗同帧不双泵；形变是派生量——面板缝
/// 停了它可能还在收，故自带活性源）。
...
pub fn fx_frame_due(now_ms: u64) -> bool {
    let active = crate::ui::seam::ai_panel_offset_y_active()
        ...
        || crate::ui::viewport_push::squash_active();
    if !active {
        LAST_FRAME_MS.store(0, Ordering::Relaxed);
        return false;
    }
