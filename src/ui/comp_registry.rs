...
pub const COMPONENTS: &[CompEntry] = &[
    ...
    CompEntry {
        ...
        spec: "宪法 §五 池行",
        tests: "tests/termview_spec.rs",
        desc: "圆角深色框行（十二修）：内芯渐变暗底（dark(c1)→dark(c2) 135°，不透明直写）。两形态：主形态 = 全包框 135° 双色渐变（左粗竖线 10px+三细边 3px 整环渐变 α255；下池目录行/下拉项/解析页 tmux 会话框与钮）；暗形态 = 无框纯渐变暗底剪影。框高 ≥3 格（2026-09-19 二轮修宪：三级框至少真·两行高）——tmux 会话框引用主形态：一行两框、内容只有 名字+×、框内局部渐变尺（页尺下整框只落渐变带一小段肉眼单色）、未附着框边框压暗表聚焦。角部渐细只渐形状不渐色（十一修）。",
    ...
    CompEntry {
        ...
        desc: "解析页第二张二级卡（2026-09-20 晚合并裁决：连接卡 + 服务卡合并成一张二级卡两竖列，省一段卡头 + 一段卡间距的纵高）。左列 = 连接：mini 卡头「连接 · 状态词」（自持在线/外部借用/连接中/退避 ×N/未启动）+ 四字段行（目标/本地口/重拉/错误）+ [重连] 钮钉左列底（杀娃重拉不等退避）；右列 = 服务：mini 卡头「服务 · 状态词」+ 四字段行（后端/在线/会话/错误）+ 每会话一行（id · 尺寸 · 空闲，死会话标死），无分隔线。文案面零改动：conn_card/svc_card 退役为文案面，合并卡继续吃同两份快照（tunnel 全局快照直读 + svc_health 三源合成，Error 相保留旧数据闪断不清卡面）。几何：两竖列等宽半分（列宽 = (内容宽−COL_GAP)/2），卡高 = PAD_V·2 + max(左列恒定, 右列(n 会话))；tmux 卡按 inset_extra_live() 预留底部带（与涂装吃同一份快照同源钉死）。复用件：池框卡环/字段标签列配方/三级框钮。",
        preview: Preview::FieldLabel,
    },
    CompEntry {
        name: "连接卡（文案面）",
