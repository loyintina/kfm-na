...
pub const COMPONENTS: &[CompEntry] = &[
    ...
    CompEntry {
        ...
        desc: "解析页二级卡（2026-09-21 三区 v2：两竖列改两段纵排——上段连接、下段服务，段内全宽；入住右上滚动区）。连接段 = mini 卡头「连接 · 状态词」（自持在线/外部借用/连接中/退避 ×N/未启动）+ 四字段行（目标/本地口/重拉/错误）+ [重连] 钮钉段底（杀娃重拉不等退避）；服务段 = mini 卡头「服务 · 状态词」+ 四字段行（后端/在线/会话/错误）+ 每会话一行（id · 尺寸 · 空闲，死会话标死），无分隔线。文案面零改动：conn_card/svc_card 退役为文案面，合并卡继续吃同两份快照（tunnel 全局快照直读 + svc_health 三源合成，Error 相保留旧数据闪断不清卡面）。几何：卡高 = PAD_V·2 + 连接段恒定 + ROW_GAP + 服务段(n 会话)；区槽位 = 卡链排布器 parser_chain::slot_rect（三区 v2：tmux 常驻右下 Dock、本卡 RightTop、环境卡 Left，滚动两本账 left/right；同日 v3：右列放宽页全区 1/2 比例制，本卡**钉顶钳高卡内滚**——「弹起后右上区域做卡片的压缩，里面内容的滑动，不要做卡片的滑动」：卡框 = 区窗内静物钉区顶、高钳进区窗，右账位移进 layout_in scroll 参卡内平移，content_clip 内芯带涂装断墨/命中闸门同一份）。复用件：池框卡环/字段标签列配方/三级框钮。",
        preview: Preview::FieldLabel,
    },
    CompEntry {
        name: "连接卡（文案面）",
