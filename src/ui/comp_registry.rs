...
pub const COMPONENTS: &[CompEntry] = &[
    ...
    CompEntry {
        ...
        spec: "宪法 §六 开合动画两件",
        tests: "tests/cfg_page_spec.rs",
        desc: "下拉面板开合（十五修+十七修两件+二十修并发）：展开 250ms ease-out 生长；点选他行 = 选中细框滑行与面板收起**同拍** 250ms ease-in-out（二十修推翻 09-14 两段时序）；点当前行/外点直接 180ms 收。细框跟面律：相交即画逐线裁剪，随面板滑出滑回不闪出。十七修：①抽屉随面——选项行/选中细框钉在全高刚体上随面板滑出滑回（下方先入场、上方先没入）；②▼三角矢量旋转 progress×180°。预览 = 语义化演示（十五修）：触发器 + ▼旋转 + 抽屉生长开合乒乓。",
    ...
    CompEntry {
        ...
        spec: "宪法 §六 面与内容一体",
        tests: "tests/cfg_page_spec.rs",
        desc: "面与内容一体平移（十七修通则，四实例）：①标签切换 = 页面级（双池框+内容整体平移，视口 = 页环）；②下池选行 = 上池级（上池内容平移，视口 = 上池框）；③下拉 = 垂直实例（抽屉随面）；④下拉换选 = 上池体级（二十修：行 0 触发器钉住，体行 1.. 平移+池高 glide，冻结窗口——并发账收尽才起步）。方向律：选择前进 = 内容左移。双代同画（旧代冻结快照带偏移出、新代活态带偏移进，禁两拍），250ms ease-in-out cubic。预览 = 语义化演示（十五修）：两个迷你页（各含双小池）整体横移换页，面与内容一体。",
