...
pub const COMPONENTS: &[CompEntry] = &[
    ...
    CompEntry {
        ...
        spec: "宪法 §五 池行",
        tests: "tests/termview_spec.rs",
        desc: "圆角深色框行（十二修）：内芯渐变暗底（dark(c1)→dark(c2) 135°，不透明直写）。两形态：主形态 = 全包框 135° 双色渐变（左粗竖线 10px+三细边 3px 整环渐变 α255；下池目录行/下拉项/解析页 tmux 会话框）；暗形态 = 无框纯渐变暗底剪影。框高最小 2 格、内容上下各留 ≥半格空隙（2026-09-19 用户修宪备注——tmux 会话框行 = 本组件主形态引用，一行两框、内容只有 名字+×）。角部渐细只渐形状不渐色（十一修）。",
