...
pub const COMPONENTS: &[CompEntry] = &[
    ...
    CompEntry {
        ...
        desc: "上池高度随内容弹簧伸缩、下池互补（十五修）：layout 逐帧侦测目标变即重定基续弹，首喂直通防开场蹦；upper_scroll 吃目标不吃瞬时值。同核 fx_spring，预览借用弹簧件曲线。",
        preview: Preview::CurveSpring,
    },
    CompEntry {
        name: "光标滑行",
        ...
        spec: "宪法 §五 池区动画两件",
        tests: "tests/cfg_page_spec.rs",
        desc: "下池选中光标跨行滑行（十五修）：像素域弹簧（SETTLE_PX=0.5 像素尺，行号域会提前 94px 贴死剪尾——实踩），行步进 189px，涂装选中全包框吃瞬时值。同核 fx_spring，预览借用弹簧件曲线。",
        preview: Preview::CurveSpring,
    },
    CompEntry {
        name: "下拉开合",
