...
//!
//! 对侧是 android/java/dev/kfm/na/KfmSurfaceView.java 的两个 static native
...
/// dev.kfm.na.KfmSurfaceView.nativeCommitText —— IME commitText 落字
/// （中文候选词、英文整串、粘贴都走这）
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_kfm_na_KfmSurfaceView_nativeCommitText(
    mut env: EnvUnowned,
    _class: JClass,
    text: JString,
) {
...
/// dev.kfm.na.KfmSurfaceView.nativeSendKey —— 软键事件（退格/回车）。
/// 接上 InputConnection 后软键盘删除走连接而非按键队列，Java 侧翻成
/// 键码送来这里，映射表在 ime_queue::key_code_to_bytes
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_kfm_na_KfmSurfaceView_nativeSendKey(
    _env: EnvUnowned,
    _class: JClass,
    code: jint,
) {
