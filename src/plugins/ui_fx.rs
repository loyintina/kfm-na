...
//!   轨迹是阶梯，纯镜像太硬。
//! - 「配置面板 X 偏移」缝（2026-09-10 面板栈 §五B）= 同吃 fx_ease 方向
...
impl Plugin for UiFx {
    ...
    fn apply(&self, ctx: &mut Ctx) -> Result<(), String> {
        ...
        // 第三道缝（2026-09-10 面板栈 §五B）：配置面板 X 偏移——左滑召唤/
        // 右滑推回，同吃 fx_ease 方向分档（入减速/出加速）
        crate::ui::seam::occupy_config_panel_offset_x(crate::ui::fx_ease::ease_occupier());
        ctx.effect(Box::new(|| {
