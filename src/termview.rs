...
impl TermView {
    ...
    /// 雾状光球（D8 拟合定稿 2026-08-30，同日改加法合成）：程序化预渲染
    /// sprite 一次（build_orb_sprite 三层 Lambert 配方的光贡献量），运行时
    /// 纯贴图饱和加——球只加光不遮光（压字反馈修复）。四态增益硬切
...
/// 三层参数化模型，常量来自 scripts/orb-fit.py 坐标下降拟合（RMSE 4.66/255，
/// 验收基准 docs/assets/orb-fit-generated.png；改动须重跑拟合器并同步
...
/// add*gain，越界裁剪。render_orb 与逐像素钉共用的同源绘制入口
