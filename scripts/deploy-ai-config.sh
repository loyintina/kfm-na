...
# 经调试闸门（na-ssh.sh：9022 首选/8024 备援，na 自己的 sshd，写私有目录的天然权限）推进
...
#
# 用法：bash scripts/deploy-ai-config.sh（手机闸不通时先 na 起来）
...
# 闸门 = na 自己的 sshd（na-ssh.sh：9022 首选/8024 备援；写私有目录的天然权限）；
# scp 依赖对端有 scp  binary，bootstrap 里不一定有——cat 重定向万能
na_ssh "mkdir -p $NA_DIR" || { echo "❌ 闸门不通——na 起来了吗？"; exit 1; }
na_ssh "cat > $NA_DIR/providers.json" < "$TMP/providers.json"
na_ssh "cat > $NA_DIR/.env && chmod 600 $NA_DIR/.env" < "$TMP/.env"

# 回读校验（形状，不验 key 值——key 不上日志）
na_ssh "ls -la $NA_DIR && head -c 80 $NA_DIR/providers.json"
