...
# 「触及测试」口径（Rust）：任意级 tests/ 目录（workspace crate 自有
# tests/ 同认——2026-09-20 扩，此前只认仓根 tests/，crates/na-server
# 的钉被误判无钉）、*_test.rs、tests.rs，或 diff 中含 #[cfg(test)]；
...
touched_tests=$(echo "$files" | grep -cE '(^|/)tests/|_test\.rs$|(^|/)tests\.rs$|^scripts/test-.*\.sh$' || true)
