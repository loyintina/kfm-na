...
> agent 入职指南。读完这份再动手。
>
> **PARADIGM v2（2026-09-21 晨班会：BAR-114 段落级→代码守卫 check-modal-cursor.sh；v1 2026-09-17 立，nz 唯一产出迭代范式 na 落地）**：本文件
...
1. **重 IO 窗口制（2026-09-01 用户收紧，取代原 22:00-07:00 版）**：
   ...
   **白天代码提交的 chain 闸 = 手机端跑（2026-09-01 用户拍板，人在场可视）**：
   `bash scripts/chain-phone.sh`（暂存区补丁推手机 apply → 全量 chain →
   落 stamp），pre-commit 白天校验 stamp（补丁哈希咬合 + 6h 有效，改码
   必重跑）；夜间窗口（01:00-07:00）提交照旧服务器本地 chain，另有
   01:43 定时任务兜底夜间重载与攒账提交。
