#!/usr/bin/env bash
# package-spike.sh — GPU 期 0 复活尖刺打包（gpu-render.md §四）
# 主包管线（scripts/package-apk.sh）的极简版：无 Java 皮无 d8 无资源，
# cargo build → aapt2 link → 装 lib（STORED + 页对齐）→ zipalign → apksigner。
# 用法：
#   bash package-spike.sh           # 打包（手机 Termux 原生编 / 服务器交叉编）
...
echo "=== [spike 1/4] cargo build（wgpu 30 复活尖刺①） ==="
...
cp "$TARGET_DIR/libgpu_spike.so" "$BUILD/stage/lib/arm64-v8a/"
...
if [ "${1:-}" = "deploy" ]; then
    ...
    echo "=== [spike] ✅ 安装器已调起：手机上点「安装」，然后开「GPU尖刺」==="
    echo "    判卷：服务器 tail -f /root/kfm-na/field-reports.log | grep gpu-spike"
fi
